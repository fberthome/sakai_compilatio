 ...
 * </p>
 * <p>
 * The Concrete Service classes extending this are the XmlFile and DbCached
 * storage classes.
 * </p>
 */
public abstract class BaseAssignmentService
		implements AssignmentService, EntityTransferrer, EntityTransferrerRefMigrator
{
	/** Our logger. */
	...
	// SAK-17606 - Property for whether an assignment uses anonymous grading
	// (user settable)
	...
	/**
	 * Access the internal reference which can be used to assess security
	 * clearance.
	 * 
	 * @param id
	...
	public String assignmentReference(String context, String id)
	{
		String retVal = null;
		if (context == null)
			retVal = getAccessPoint(true) + Entity.SEPARATOR + "a" + Entity.SEPARATOR
					+ id;
		else
			retVal = getAccessPoint(true) + Entity.SEPARATOR + "a" + Entity.SEPARATOR
	...
	/**
	 * I feel silly having to look up the entire assignment object just to get
	 * the reference, but if there's no context, that seems to be the only way
	 * to do it.
	 * 
	 * @param id
	 * @return
	...
	public String assignmentReference(String id)
	{
		...
		if (assignment != null) ref = assignment.getReference();
		return ref;
	} // assignmentReference
	...
	public List getSortedGroupUsers(Group _g)
	{
		...
		while (_members.hasNext())
		{
			...
			{
				M_log.warn(" BaseAssignmentSubmission Group getSubmitters"
						+ e.getMessage() + _member.getUserId());
			}
		}
	...
	public String contentReference(String context, String id)
	{
		String retVal = null;
		if (context == null)
			retVal = getAccessPoint(true) + Entity.SEPARATOR + "c" + Entity.SEPARATOR
					+ id;
		else
			retVal = getAccessPoint(true) + Entity.SEPARATOR + "c" + Entity.SEPARATOR
					+ context + Entity.SEPARATOR + id;
	...
	public String submissionReference(String context, String id, String assignmentId)
	{
		String retVal = null;
		if (context == null)
			retVal = getAccessPoint(true) + Entity.SEPARATOR + "s" + Entity.SEPARATOR
					+ id;
		else
			retVal = getAccessPoint(true) + Entity.SEPARATOR + "s" + Entity.SEPARATOR
					+ context + Entity.SEPARATOR + assignmentId + Entity.SEPARATOR + id;
		return retVal;
	 ...
	 *        - The lock id string.
	 * @param resource
	 *        - The resource reference string, or null if no resource is
	 *        involved.
	 ...
	 * 
	 * @param lock
	 *        The security function to be checked, 'asn.submit' for example.
	 * @param resource
	 *        The resource to be accessed
	 * @param assignment
	 *        An Assignment object. We use this for the group checks.
	 * @return
	 */
	protected boolean unlockCheckWithGroups(String lock, String resource,
			Assignment assignment)
	{
		...
		if (allowAllGroups(context)
				&& securityService.unlock(lock, SiteService.siteReference(context)))
		{
			return true;
		}
		...
		if (groupIds != null && groupIds.size() > 0)
		{
			...
			while (i.hasNext())
			{
				...
				boolean isAllowed = securityService.unlock(lock, groupId);
	...
	protected boolean unlockCheck2(String lock1, String lock2, String resource)
	{
		...
		if ((!lock1.equals(lock2)) && (securityService.unlock(lock2, resource)))
			return true;
	...
	protected void unlock(String lock, String resource) throws PermissionException
	{
		...
		{
			throw new PermissionException(SessionManager.getCurrentSessionUserId(), lock,
					resource);
		}
	 ...
	 * @param value
	 *        true or false
	 * @deprecated 7 April 2014 - this has no effect anymore and should be
	 *             removed in 11 release
	 */
	 ...
	 */
	public void setAssignmentActivityProducer(
	 ...
	 */
	public void setAllowGroupAssignmentsInGradebook(
	...
	public void init()
	{
		...
		m_allowSubmitByInstructor = m_serverConfigurationService
				.getBoolean("assignments.instructor.submit.for.student", true);
	 ...
	 * 
	 * @param context
	 *        - Describes the portlet context - generated with
	 *        DefaultId.getChannel().
	...
	public AssignmentEdit addAssignment(String context) throws PermissionException
	{
		...
		// event for tracking
		((BaseAssignmentEdit) assignment)
				.setEvent(AssignmentConstants.EVENT_ADD_ASSIGNMENT);
	...
	/**
	 * Add a new assignment to the directory, from a definition in XML. Must
	 ...
	 * @exception PermissionException
	 *            if the current user does not have permission to add an
	 *            assignnment.
	 */
			...
			throws IdInvalidException, IdUsedException, PermissionException
	{
		...
		if (!Validator.checkResourceId(assignmentFromXml.getId()))
			throw new IdInvalidException(assignmentFromXml.getId());

		// check security (throws if not permitted)
		unlock(SECURE_ADD_ASSIGNMENT, assignmentFromXml.getReference());

		// reserve a assignment with this id from the info store - if it's in
		// use, this will return null
		AssignmentEdit assignment = m_assignmentStorage.put(assignmentFromXml.getId(),
				assignmentFromXml.getContext());
		if (assignment == null)
		{
		...
		ResourcePropertiesEdit propertyEdit = (BaseResourcePropertiesEdit) assignment
				.getProperties();
		try
		{
		...
		{
			M_log.error(
					this + " mergeAssignment error when trying to get creation time property "
							+ epte);
		}
	...
	/**
	 * Creates and adds a new Assignment to the service which is a copy of an
	 * existing Assignment.
	 * 
	 * @param assignmentId
	 *        - The Assignment to be duplicated.
	 * @return The new Assignment object, or null if the original Assignment
	 *         does not exist.
	 * @throws PermissionException
	 *         if current User does not have permission to do this.
	 */
	public AssignmentEdit addDuplicateAssignment(String context,
			String assignmentReference) throws PermissionException, IdInvalidException,
			IdUsedException, IdUnusedException
	{

		M_log.debug(this + " ENTERING ADD DUPLICATE ASSIGNMENT WITH ID : "
				+ assignmentReference);
		...
		if (assignmentReference != null)
		{
			...
			else
			{
				...
				M_log.debug(
						this + " addDuplicateAssignment : assignment exists - will copy");

				Assignment existingAssignment = getAssignment(assignmentReference);
				newContent = addDuplicateAssignmentContent(context,
						existingAssignment.getContentReference());
				commitEdit(newContent);

				retVal = addAssignment(context);
				retVal.setContentReference(newContent.getReference());
				retVal.setTitle(existingAssignment.getTitle() + " - "
						+ rb.getString("assignment.copy"));
				...
				ResourcePropertiesEdit pEdit = (BaseResourcePropertiesEdit) retVal
						.getProperties();
		...
		M_log.debug(
				this + " ADD DUPLICATE ASSIGNMENT : LEAVING ADD DUPLICATE ASSIGNMENT WITH ID : "
	 ...
	 * @return The Assignment corresponding to the reference, or null if it does
	 *         not exist.
	...
	/**
	 * Check visibility of an assignment for a given user. We consider an an
	 * assignment to be visible to the user if it has been opened and is not
	 * deleted. However, we allow access to deleted assignments if the user has
	 * already made a submission for the assignment. Note that this method does
	 * not check permissions at all. It should already be established that the
	 * user is permitted to access this assignment.
	 *
	 * @param assignment
	...
	private boolean isAvailableOrSubmitted(Assignment assignment, String userId)
	{
		...
		String deleted = assignment.getProperties()
				.getProperty(ResourceProperties.PROP_ASSIGNMENT_DELETED);
		if (deleted == null || "".equals(deleted))
		{
			...
			if (((openTime != null && TimeService.newTime().after(openTime))
		...
		{
			// and those deleted but not non-electronic assignments but the user
			// has made submissions to them
			accessible = true;
		}
		return accessible;
	}

	private Assignment checkAssignmentAccessibleForUser(Assignment assignment,
			String currentUserId) throws PermissionException
	{
		...
		if (assignment.getAccess() == Assignment.AssignmentAccess.GROUPED)
		{
			...
			Collection<Group> allowedGroups = getGroupsAllowGetAssignment(context,
					currentUserId);
			// reject and throw PermissionException if there is no intersection
			if (!allowAllGroups(context) && !currentUserIsCreator(assignment)
					&& !isIntersectionGroupRefsToGroups(asgGroups, allowedGroups))
			{
				throw new PermissionException(currentUserId, SECURE_ACCESS_ASSIGNMENT,
	...
	/**
	 * Access all assignment objects - known to us (not from external
	 * providers).
	 * 
	...
	private List assignments(String context, String userId)
	{
		...
		else
		{
			...
			// check for the site and group permissions of these assignments as
			// well as visibility (release time, etc.)
	...
	/**
	 * Access all assignment objects for a site without considering user
	 * permissions. This should be used with care; almost all scenarios should
	 * use {@link getAssignments(String)} or {@link getAssignments(String,
	 * String)}, which do enforce permissions and visibility.
	 *
	 * @return A list of Assignment objects.
	 */
	 ...
	 * set-based efficiency over iteration in building a list of assignments for
	 * a given user. There are a few ways that we consider an assignment to be
	 * accessible: 1. The user can add assignments to the site, or 2. The
	 * assignment is grouped and the user can view assignments in at least one
	 * of those groups, or 3. The assignment is ungrouped and the user can view
	 * assignments in the site An additional state check applies, which is that
	 * the assignment is not visible if it is deleted, except when the user has
	 * made a submission for it already or can add (manage) assignments. These
	 * rules were extracted from assignments() and we are enforcing them here
	 * for a set, rather than a single assignment. This is a somewhat awkward
	 * signature; it should really either have just the assignments list or just
	 * the siteId, but the other methods are not refactored now. Namely,
	 * getAssignments calls assignments, which has some cache specifics and
	 * other items that would need to be refactored very carefully. Rather than
	 * potentially changing the behavior subtly, this only replaces the
	 * iterative permissions checks with set-based ones.
	 *
	 * @param assignments
	 *        a list of assignments to filter; must all be from the same site
	 * @param siteId
	 *        the Site ID for all assignments
	 * @param userId
	 *        the user whose access should be checked for the assignments
	 * @return a list of the assignments that are accessible; will never be null
	 *         but may be empty
	 */
	protected List<Assignment> getAccessibleAssignments(List<Assignment> assignments,
			String siteId, String userId)
	{
		...
		// Partition the assignments into grouped and ungrouped for access
		// checks
		...
		boolean allowSiteGet = securityService.unlock(userId, SECURE_ACCESS_ASSIGNMENT,
				siteRef);
		if (allowSiteGet)
		{
		...
		List<Assignment> visible = (securityService.unlock(userId, SECURE_ADD_ASSIGNMENT,
				siteRef)) ? permitted : filterAssignmentsByVisibility(permitted, userId);

		// We are left with the original list filtered by site/group permissions
		// and visibility/submission state
		return visible;
	}
	 ...
	 *
	 * @param assignments
	 *        the list of assignments to filter; none may be null
	 * @param siteId
	 *        the site ID to use to filter
	 ...
	 */
	protected List<Assignment> filterAssignmentsBySite(List<Assignment> assignments,
			String siteId)
	{
	...
	/**
	 * Partition a list of assignments into those that are grouped and
	 * ungrouped.
	 *
	 * @param assignments
	 *        the list of assignments to inspect and partition
	 ...
	 *         never null, always two elements, neither list is null; any null
	 *         assignments will be omitted in the final lists
	 */
	protected List<List<Assignment>> partitionAssignments(List<Assignment> assignments)
	{
		...
		{
			if (assignment != null
					&& assignment.getAccess() == Assignment.AssignmentAccess.GROUPED)
			{
	...
	/**
	 * Filter a list of grouped assignments by permissions based on a given
	 * site. Note that this does not consider the assignment or submission
	 * state, only permissions.
	 *
	 * @param assignments
	 *        the list of assignments to filter; should all be grouped and from
	 *        the same site
	 * @param siteId
	 *        the site to which all of the assignments belong
	 * @param userId
	 *        the user for which group permissions should be checked
	 * @return a new list of assignments, containing those supplied that the
	 *         user can access, based on permission to view the assignment in
	 *         one or more of its groups, permission to add assignments in the
	 *         site, or permission to view assignments in all of the site's
	 *         groups; never null but may be empty
	 */
	protected List<Assignment> filterGroupedAssignmentsForAccess(
			List<Assignment> assignments, String siteId, String userId)
	{
		List<Assignment> filtered = new ArrayList<Assignment>();

		// Short-circuit to save the group query if we can't make a reasonable
		// check
		if (assignments == null || assignments.isEmpty() || siteId == null
				|| userId == null)
		{
			return filtered;
		}
		...
		Collection<Group> allowedGroups = (Collection<Group>) getGroupsAllowGetAssignment(
				siteId, userId);
	...
	private boolean currentUserIsCreator(Assignment assignment)
	{
		...
		if (assignmentCreator != null && assignmentCreator
	...
	/**
	 * Filter a list of assignments based on visibility (open time, deletion,
	 * submission, etc.) for a specified user. Note that this only considers
	 * assignment and submission state and does not consider permissions so the
	 * assignments should have already been checked for permissions for the
	 * given user.
	 *
	 * @param assignments
	 *        the list of assignments to filter
	 * @param userId
	 *        the user for whom to check visibility; should be permitted to
	 *        access all of the assignments
	 * @return a new list containing those supplied that the user may access,
	 *         based on visibility; never null but may be empty
	 */
	protected List<Assignment> filterAssignmentsByVisibility(List<Assignment> assignments,
			String userId)
	{
	...
	/**
	 * See if the collection of group reference strings has at least one group
	 * that is in the collection of Group objects.
	 * 
	 * @param groupRefs
	...
	/**
	 * Get a locked assignment object for editing. Must commitEdit() to make
	 ...
	 * @exception PermissionException
	 *            if the current user does not have permission to edit this
	 *            assignment.
			...
			throws IdUnusedException, PermissionException, InUseException
	{
		...
		((BaseAssignmentEdit) assignmentEdit)
				.setEvent(AssignmentConstants.EVENT_UPDATE_ASSIGNMENT);
	...
	/**
	 * Commit the changes made to an AssignmentEdit object, and release the
	 * lock.
	 * 
	 * @param assignment
	...
	public void commitEdit(AssignmentEdit assignment)
	{
		...
		if (!assignment.isActiveEdit())
		{
			...
			{
				M_log.warn(" commitEdit(): closed AssignmentEdit " + e.getMessage()
		...
		EventTrackingService.post(EventTrackingService.newEvent(
				((BaseAssignmentEdit) assignment).getEvent(), assignment.getReference(),
				true));
	...
	public void cancelEdit(AssignmentEdit assignment)
	{
		...
		if (!assignment.isActiveEdit())
		{
			...
			{
				M_log.warn(" cancelEdit(): closed AssignmentEdit " + e.getMessage()
						+ " assignment id=" + assignment.getId());
			}
			return;
		}
	...
	public void removeAssignment(AssignmentEdit assignment) throws PermissionException
	{
		if (assignment != null)
		{
			...
			if (!assignment.isActiveEdit())
			{
				...
				{
					M_log.warn(" removeAssignment(): closed AssignmentEdit"
							+ e.getMessage() + " assignment id=" + assignment.getId());
				}
				return;
			}
			...
			{
				M_log.warn(" removeAssignment: removing realm for assignment reference="
	 ...
	 * {@inheritDoc}
	 */
	public void removeAssignmentAndAllReferences(AssignmentEdit assignment)
			throws PermissionException
	{
		if (assignment != null)
		{
			M_log.debug(this + " removeAssignmentAndAllReferences with id : "
					+ assignment.getId());
			...
			if (!assignment.isActiveEdit())
			{
				...
				{
					M_log.warn(
							" removeAssignmentAndAllReferences(): closed AssignmentEdit"
									+ e.getMessage() + " assignment id="
									+ assignment.getId());
				}
				return;
			}
			...
			// we may need to remove associated calendar events and annc, so get
			// the basic info here
			...
			removeAssociatedAnnouncementItem(getAnnouncementChannel(context), assignment,
					pEdit);
			...
			if (submissions != null)
			{
				for (Iterator sIterator = submissions.iterator(); sIterator.hasNext();)
				{
					...
					{
						M_log.warn(
								"removeAssignmentAndAllReference: User does not have permission to remove submission "
										+ sReference + " for assignment: "
										+ assignment.getId() + e.getMessage());
					}
					catch (InUseException e)
					{
						M_log.warn("removeAssignmentAndAllReference: submission "
								+ sReference + " for assignment: " + assignment.getId()
								+ " is in use. " + e.getMessage());
					}
					catch (IdUnusedException e)
					{
						M_log.warn("removeAssignmentAndAllReference: submission "
			...
			try
			{
				removeAssignmentContent(
						editAssignmentContent(assignment.getContent().getReference()));
			}
			catch (AssignmentContentNotEmptyException e)
			{
				M_log.warn(
						" removeAssignmentAndAllReferences(): cannot remove non-empty AssignmentContent object for assignment = "
								+ assignment.getId() + ". " + e.getMessage());
			}
			catch (PermissionException e)
			{
				M_log.warn(
						" removeAssignmentAndAllReferences(): not allowed to remove AssignmentContent object for assignment = "
								+ assignment.getId() + ". " + e.getMessage());
			}
			catch (InUseException e)
			{
				M_log.warn(
						" removeAssignmentAndAllReferences(): AssignmentContent object for assignment = "
								+ assignment.getId() + " is in used. " + e.getMessage());
			}
			catch (IdUnusedException e)
			{
				M_log.warn(
						" removeAssignmentAndAllReferences(): cannot find AssignmentContent object for assignment = "
								+ assignment.getId() + ". " + e.getMessage());
			}
	...
	private void removeAssociatedTaggingItem(AssignmentEdit assignment)
	{
		try
		{
			if (m_taggingManager.isTaggable())
			{
				...
				{
					provider.removeTags(
							m_assignmentActivityProducer.getActivity(assignment));
				}
			}
		}
		...
		{
			M_log.warn(
					"removeAssociatedTaggingItem: User does not have permission to remove tags for assignment: "
	 ...
	 * @param context
	 */
	private void removeAssociatedGradebookItem(ResourcePropertiesEdit pEdit,
			String context)
	{
		String associatedGradebookAssignment = pEdit.getProperty(
		...
		if (associatedGradebookAssignment != null)
		{
			...
			{
				m_gradebookExternalAssessmentService.removeExternalAssessment(context,
	...
	private Calendar getCalendar(String contextId)
	{
		...
		if (calendarId == null)
		{
			calendarId = m_calendarService.calendarReference(contextId,
			...
			{
				M_log.warn(
						"getCalendar: The current user does not have permission to access "
								+ "the calendar for context: " + contextId,
						e);
			}
			catch (Exception ex)
			{
				M_log.warn(
						"getCalendar: Unknown exception occurred retrieving calendar for site: "
								+ contextId,
						ex);
				calendar = null;
			}
	...
	/**
	 * Will determine if there is a calendar event associated with this
	 * assignment and remove it, if found.
	 * 
	 * @param calendar
	 *        Calendar
	 * @param aEdit
	 ...
	 */
	private void removeAssociatedCalendarItem(Calendar calendar, AssignmentEdit aEdit,
			ResourcePropertiesEdit pEdit)
	{
		...
		if (isThereEvent != null && isThereEvent.equals(Boolean.TRUE.toString()))
		{
			...
			if (calendar != null)
			{
				...
				String oldEventId = pEdit.getProperty(
				...
				if (event != null)
				{
					try
					{
						calendar.removeEvent(calendar.getEditEvent(event.getId(),
								CalendarService.EVENT_REMOVE_CALENDAR));
						pEdit.removeProperty(NEW_ASSIGNMENT_DUE_DATE_SCHEDULED);
						pEdit.removeProperty(
								ResourceProperties.PROP_ASSIGNMENT_DUEDATE_CALENDAR_EVENT_ID);
					}
					catch (PermissionException ee)
					{
						M_log.warn(
								":removeCalendarEvent not allowed to remove calendar event for assignment = "
										+ aEdit.getTitle() + ". ");
					}
					catch (InUseException ee)
					{
						M_log.warn(
								":removeCalendarEvent someone else is editing calendar event for assignment = "
										+ aEdit.getTitle() + ". ");
					}
					catch (IdUnusedException ee)
					{
						M_log.warn(
								":removeCalendarEvent calendar event are in use for assignment = "
										+ aEdit.getTitle() + " and event ="
										+ event.getId());
					}
				}
			}
	...
	private AnnouncementChannel getAnnouncementChannel(String contextId)
	{
		...
		String channelId = m_serverConfigurationService
				.getString(m_announcementService.ANNOUNCEMENT_CHANNEL_PROPERTY, null);
		if (channelId == null)
		{
			channelId = m_announcementService.channelReference(contextId,
					SiteService.MAIN_CONTAINER);
			try
			{
	...
	/**
	 * Will determine if there is an announcement associated with this
	 * assignment and removes it, if found.
	 * 
	 * @param channel
	 *        AnnouncementChannel
	 * @param aEdit
	 ...
	 *        ResourcePropertiesEdit
	 */
	private void removeAssociatedAnnouncementItem(AnnouncementChannel channel,
			AssignmentEdit aEdit, ResourcePropertiesEdit pEdit)
	{
		if (channel != null)
		{
			String openDateAnnounced = StringUtils
					.trimToNull(pEdit.getProperty("new_assignment_open_date_announced"));
			String openDateAnnouncementId = StringUtils.trimToNull(pEdit.getProperty(
			...
			throws PermissionException
	{
		...
		{
			throw new PermissionException(SessionManager.getCurrentSessionUserId(),
					SECURE_ADD_ASSIGNMENT_CONTENT, contentId);
		}
		...
		// event for tracking
		((BaseAssignmentContentEdit) content)
				.setEvent(AssignmentConstants.EVENT_ADD_ASSIGNMENT_CONTENT);
			...
			throws IdInvalidException, IdUsedException, PermissionException
	{
		...
		// check for a valid assignment name
		if (!Validator.checkResourceId(contentFromXml.getId()))
			throw new IdInvalidException(contentFromXml.getId());

		// check security (throws if not permitted)
		unlock(SECURE_ADD_ASSIGNMENT_CONTENT, contentFromXml.getReference());

		// reserve a content with this id from the info store - if it's in use,
		// this will return null
		AssignmentContentEdit content = m_contentStorage.put(contentFromXml.getId(),
				contentFromXml.getContext());
		if (content == null)
		{
			throw new IdUsedException(contentFromXml.getId());
		}

		// transfer from the XML read content object to the
	...
	/**
	 * Creates and adds a new AssignmentContent to the service which is a copy
	 * of an existing AssignmentContent.
	 * 
	 * @param context
	 *        - From DefaultId.getChannel(RunData)
	 * @param contentReference
	 *        - The id of the AssignmentContent to be duplicated.
	 * @return AssignmentContentEdit The new AssignmentContentEdit object, or
	 *         null if the original does not exist.
	 * @throws PermissionException
	 *         if current User does not have permission to do this.
	 */
			...
			throws PermissionException, IdInvalidException, IdUnusedException
	{
		...
		M_log.debug(this + " ENTERING ADD DUPLICATE ASSIGNMENT CONTENT : "
				+ contentReference);
		...
		if (contentReference != null)
		{
			...
			else
			{
				...
				retVal.setTitle(existingContent.getTitle() + " - "
						+ rb.getString("assignment.copy"));
				...
				if (tempVector != null)
				{
					for (int z = 0; z < tempVector.size(); z++)
					{
						...
						if (tempRef != null)
						{
							...
							String tempRefCollectionId = m_contentHostingService
									.getContainingCollectionId(tempRefId);
							try
							{
								// get the original attachment display name
								ResourceProperties p = m_contentHostingService
										.getProperties(tempRefId);
								String displayName = p.getProperty(
										ResourceProperties.PROP_DISPLAY_NAME);
								// add another attachment instance
								String newItemId = m_contentHostingService
										.copyIntoFolder(tempRefId, tempRefCollectionId);
								ContentResourceEdit copy = m_contentHostingService
										.editResource(newItemId);
								// with the same display name
								ResourcePropertiesEdit pedit = copy.getPropertiesEdit();
								pedit.addProperty(ResourceProperties.PROP_DISPLAY_NAME,
										displayName);
								m_contentHostingService.commitResource(copy,
										NotificationService.NOTI_NONE);
								newRef = m_entityManager
										.newReference(copy.getReference());
								retVal.addAttachment(newRef);
							}
							catch (Exception e)
							{

								M_log.warn(" LEAVING ADD DUPLICATE CONTENT : "
										+ e.toString());
							}
						}
					}
		...
		M_log.debug(this + " LEAVING ADD DUPLICATE CONTENT WITH ID : " + retVal != null
				? retVal.getId() : "");
	 ...
	 * @return The AssignmentContent corresponding to the reference, or null if
	 *         it does not exist.
	 * @throws IdUnusedException
	 *         if there is no object with this reference.
	 * @throws PermissionException
			...
			throws IdUnusedException, PermissionException
	{
		...
		// track event
		// EventTrackingService.post(EventTrackingService.newEvent(AssignmentConstants.EVENT_ACCESS_ASSIGNMENT_CONTENT,
		// content.getReference(), false));
	...
	/**
	 * Access all AssignmentContent objects - known to us (not from external
	 * providers).
	 * 
	...
	/**
	 * Get a locked AssignmentContent object for editing. Must commitEdit() to
	 * make official, or cancelEdit() when done!
	 * 
	 * @param id
	 *        The content id string.
			...
			throws IdUnusedException, PermissionException, InUseException
	{
		...
		// ignore the cache - get the AssignmentContent with a lock from the
		// info store
		AssignmentContentEdit content = m_contentStorage.edit(contentId);
		if (content == null) throw new InUseException(contentId);

		((BaseAssignmentContentEdit) content)
				.setEvent(AssignmentConstants.EVENT_UPDATE_ASSIGNMENT_CONTENT);
	...
	/**
	 * Commit the changes made to an AssignmentContentEdit object, and release
	 * the lock.
	 * 
	 * @param content
	...
	public void commitEdit(AssignmentContentEdit content)
	{
		...
		// track it
		EventTrackingService.post(EventTrackingService.newEvent(
				((BaseAssignmentContentEdit) content).getEvent(), content.getReference(),
				true));
	...
	/**
	 * Cancel the changes made to a AssignmentContentEdit object, and release
	 * the lock.
	 * 
	 * @param content
	 *        The AssignmentContentEdit object to commit.
	...
	public void cancelEdit(AssignmentContentEdit content)
	{
		...
		if (!content.isActiveEdit())
		{
			...
			{
				M_log.warn(" cancelEdit(): closed AssignmentContentEdit " + e.getMessage()
						+ " assignment content id=" + content.getId());
			}
			return;
		}
	 ...
	 * @throws an
	 *         AssignmentContentNotEmptyException if this content still has
	 *         related Assignments.
	 * @throws PermissionException
	 *         if current User does not have permission to do this.
	 */
			...
			throws AssignmentContentNotEmptyException, PermissionException
	{
		if (content != null)
		{
			if (!content.isActiveEdit())
			{
				...
				{
					M_log.warn(" removeAssignmentContent(): closed AssignmentContentEdit "
							+ e.getMessage() + " assignment content id="
							+ content.getId());
				}
				return;
			}
	...
	public AssignmentSubmissionEdit addSubmission(String context, String assignmentId,
			String submitterId) throws PermissionException
	{
		...
		{
			throw new PermissionException(SessionManager.getCurrentSessionUserId(),
					SECURE_ADD_ASSIGNMENT_SUBMISSION, key);
		}
		...
		AssignmentSubmissionEdit submission = m_submissionStorage.put(submissionId,
				assignmentId, submitterId, null, null, null);

		if (submission != null)
		{
			submission.setContext(context);
			// event for tracking
			((BaseAssignmentSubmissionEdit) submission)
					.setEvent(AssignmentConstants.EVENT_ADD_ASSIGNMENT_SUBMISSION);
			M_log.debug(this + " LEAVING ADD SUBMISSION : REF : "
					+ submission.getReference());
		}
		else
		{
			M_log.warn(
					this + " ADD SUBMISSION: cannot add submission object with submission id="
							+ submissionId + ", assignment id=" + assignmentId
							+ ", and submitter id=" + submitterId);
		}
		return submission;
	}

	/**
	 * Add a new AssignmentSubmission to the directory, from a definition in
	 * XML. Must commitEdit() to make official, or cancelEdit() when done!
	 * 
	 * @param el
	 ...
	 * @exception PermissionException
	 *            if the current user does not have permission to add a
	 *            submission.
	 */
			...
			throws IdInvalidException, IdUsedException, PermissionException
	{
		...
		if (!Validator.checkResourceId(submissionFromXml.getId()))
			throw new IdInvalidException(submissionFromXml.getId());

		// check security (throws if not permitted)
		unlock(SECURE_ADD_ASSIGNMENT_SUBMISSION, submissionFromXml.getReference());

		// reserve a submission with this id from the info store - if it's in
		// use, this will return null
		AssignmentSubmissionEdit submission = m_submissionStorage
				.put(submissionFromXml.getId(), submissionFromXml.getAssignmentId(),
						submissionFromXml.getSubmitterIdString(),
						(submissionFromXml.getTimeSubmitted() != null)
								? String.valueOf(
										submissionFromXml.getTimeSubmitted().getTime())
								: null,
	...
	/**
	 * Get a locked AssignmentSubmission object for editing. Must commitEdit()
	 * to make official, or cancelEdit() when done!
	 * 
			...
			throws IdUnusedException, PermissionException, InUseException
	{
		...
		// ignore the cache - get the AssignmentSubmission with a lock from the
		// info store
		...
		if (!unlockCheck(SECURE_GRADE_ASSIGNMENT_SUBMISSION, submissionReference)
				&& !unlockCheck(SECURE_UPDATE_ASSIGNMENT, submissionReference))
		{
			...
			if (unlockCheck(SECURE_UPDATE_ASSIGNMENT_SUBMISSION, submissionReference))
			{
				...
				if (a.isGroup())
				{
					...
					try
					{
						notAllowed = st.getGroup(submission.getSubmitterId())
								.getMember(currentUser.getId()) == null;
					}
				...
				else
				{
							...
							&& submission.getSubmitterId().equals(currentUser.getId()))
					{
						...
						// then test against extra criteria depend on the status
						// of submission
						try
						{
						...
						{
							M_log.warn(
									" editSubmission(): cannot get assignment for submission "
											+ submissionReference + e.getMessage());
						}
					}
				}
			}
			if (notAllowed)
			{
				...
				throw new PermissionException(currentUser.getId(),
						SECURE_UPDATE_ASSIGNMENT, submissionReference);
			}
		...
		((BaseAssignmentSubmissionEdit) submission)
				.setEvent(AssignmentConstants.EVENT_UPDATE_ASSIGNMENT_SUBMISSION);
	...
	/**
	 * Commit the changes made to an AssignmentSubmissionEdit object, and
	 * release the lock.
	 * 
	 * @param submission
	...
	public void commitEdit(AssignmentSubmissionEdit submission)
	{
		...
		if (!submission.isActiveEdit())
		{
			...
			{
				M_log.warn(
						" commitEdit(): closed AssignmentSubmissionEdit assignment submission id="
		...
		try
		{
			...
			else if (returnedTime == null && !s.getReturned()
					&& (submittedTime == null /* grading non-submissions */
							|| (submittedTime != null && (s.getTimeLastModified()
									.getTime() - submittedTime.getTime()) > 1000
											* 60 /*
													 * make sure the last
													 * modified time is at least
													 * one minute after the
													 * submit time
													 */)))
			{
				if (StringUtils.trimToNull(s.getSubmittedText()) == null
						&& s.getSubmittedAttachments().isEmpty()
						&& StringUtils.trimToNull(s.getGrade()) == null
						&& StringUtils.trimToNull(s.getFeedbackText()) == null
						&& StringUtils.trimToNull(s.getFeedbackComment()) == null
						&& s.getFeedbackAttachments().isEmpty())
				{
						...
						for (User user : s.getSubmitters())
						{
							lrss.registerStatement(
									getStatementForAssignmentGraded(
											lrss.getEventActor(event), event, a, s, user),
									"assignment");
						}
					}
				}
			}
			else if (returnedTime != null && s.getGraded()
					&& (submittedTime == null/* returning non-submissions */
							|| (submittedTime != null && returnedTime
									.after(submittedTime))/*
															 * returning normal
															 * submissions
															 */
							|| (submittedTime != null && submittedTime.after(returnedTime)
									&& s.getTimeLastModified().after(
											submittedTime))/*
															 * grading the
															 * resubmitted
															 * assignment
															 */))
			{
				// releasing a submitted assignment or releasing grade to an
				...
				if (null != lrss && StringUtils.isNotEmpty(s.getGrade()))
				{
					...
					{
						lrss.registerStatement(
								getStatementForAssignmentGraded(lrss.getEventActor(event),
										event, a, s, user),
								"assignment");
					}
				}

				// if this is releasing grade, depending on the release grade
				// notification setting, send email notification to student
				sendGradeReleaseNotification(s.getGradeReleased(),
						a.getProperties().getProperty(
								Assignment.ASSIGNMENT_RELEASEGRADE_NOTIFICATION_VALUE),
						s.getSubmitters(), s);
				if (resubmitNumber != null)
					sendGradeReleaseNotification(s.getGradeReleased(),
							a.getProperties().getProperty(
									Assignment.ASSIGNMENT_RELEASERESUBMISSION_NOTIFICATION_VALUE),
							s.getSubmitters(), s);
			}
			else if (submittedTime == null) /* grading non-submission */
			{
				...
				if (null != lrss)
				{
					...
					{
						lrss.registerStatement(
								getStatementForUnsubmittedAssignmentGraded(
										lrss.getEventActor(event), event, a, s, user),
								"sakai.assignment");
					}
				}
			...
			else
			{
						...
						.getTypeOfSubmission() != Assignment.NON_ELECTRONIC_ASSIGNMENT_SUBMISSION)
				{
					...
					// student notification, whether the student gets email
					// notification once he submits an assignment
	...
	protected void sendGradeReleaseNotification(boolean released,
			String notificationSetting, User[] allSubmitters, AssignmentSubmission s)
	{
		if (allSubmitters == null) return;

		// SAK-19916 need to filter submitters against list of valid users still
		// in site
		...
		if (released && notificationSetting != null && notificationSetting
				.equals(Assignment.ASSIGNMENT_RELEASEGRADE_NOTIFICATION_EACH))
		{
			// send email to every submitters
			if (submitters != null)
			{
				// send the message immidiately
				EmailService.sendToUsers(new ArrayList(Arrays.asList(submitters)),
						getHeaders(null, "releasegrade"),
						getNotificationMessage(s, "releasegrade"));
			}
		}
		if (notificationSetting != null && notificationSetting
				.equals(Assignment.ASSIGNMENT_RELEASERESUBMISSION_NOTIFICATION_EACH))
		{
			// send email to every submitters
			if (submitters != null)
			{
				// send the message immidiately
				EmailService.sendToUsers(new ArrayList(Arrays.asList(submitters)),
						getHeaders(null, "releaseresumbission"),
						getNotificationMessage(s, "releaseresumbission"));
			}
		}
	}
	...
	{
		String notiOption = a.getProperties()
				.getProperty(Assignment.ASSIGNMENT_INSTRUCTOR_NOTIFICATIONS_VALUE);
		if (notiOption != null && !notiOption
				.equals(Assignment.ASSIGNMENT_INSTRUCTOR_NOTIFICATIONS_NONE))
		{
			...
			// of users who can actually grade this assignment
			...
			if (notiOption.equals(Assignment.ASSIGNMENT_INSTRUCTOR_NOTIFICATIONS_EACH))
			{
				...
				EmailService.sendToUsers(receivers, getHeaders(null, "submission"),
						messageBody);
			}
			else if (notiOption
					.equals(Assignment.ASSIGNMENT_INSTRUCTOR_NOTIFICATIONS_DIGEST))
			{
				...
				for (Iterator iReceivers = receivers.iterator(); iReceivers.hasNext();)
				{
					...
					DigestService.digest(user.getId(), getSubject("submission"),
							digestMsgBody);
				}
			}
		}
	...
	protected String getPlainTextNotificationMessage(AssignmentSubmission s,
			String submissionOrReleaseGrade)
	{
	...
	{
		if (m_serverConfigurationService
				.getBoolean("assignment.submission.confirmation.email", true))
		{
			// send notification
			...
			EmailService.sendToUsers(receivers, getHeaders(null, "submission"),
					getNotificationMessage(s, "submission"));
		}
	}
			...
			String submissionOrReleaseGrade)
	{
		if ("submission".equals(submissionOrReleaseGrade))
			...
			return FormattedText
					.convertFormattedTextToPlaintext(htmlContentReleaseGrade(s));
		else
			return FormattedText
					.convertFormattedTextToPlaintext(htmlContentReleaseResubmission(s));
	}
	...
	private String htmlContent(AssignmentSubmission s)
	{
		...
		buffer.append(rb.getString("noti.assignment.duedate") + " "
				+ a.getDueTime().toStringLocalFull() + newline + newline);
		...
		buffer.append(rb.getString("noti.submit.time") + " "
				+ s.getTimeSubmitted().toStringLocalFull() + newline + newline);

		// submit text
		String text = StringUtils.trimToNull(s.getSubmittedText());
		if (text != null)
		{
			buffer.append(rb.getString("gen.submittedtext") + newline + newline
		...
		{
			if (a.getContent()
					.getTypeOfSubmission() == Assignment.SINGLE_ATTACHMENT_SUBMISSION)
			{
			...
			for (int j = 0; j < attachments.size(); j++)
			{
				...
				buffer.append(r.getProperties()
						.getProperty(ResourceProperties.PROP_DISPLAY_NAME) + " ("
						+ r.getProperties().getPropertyFormatted(
								ResourceProperties.PROP_CONTENT_LENGTH)
						+ ")\n");
	...
	private Object getArchiveManifest(Reference r)
	{
		...
		if (".zip".equals(extension))
		{
			...
			while (it.hasNext())
			{
				...
				builder.append(entry.getKey() + " (" + formatFileSize(entry.getValue())
						+ ")" + newline);
			}
		}
	...
	private String getFileExtension(Reference r)
	{
		...
		String fileName = resourceProperties
				.getProperty(resourceProperties.getNamePropDisplayName());
	...
	private String htmlContentReleaseGrade(AssignmentSubmission s)
	{
		...
		{
			M_log.warn(" htmlContentReleaseGrade(), site id =" + context + " "
					+ ee.getMessage());
		}
		...
		buffer.append(rb.getString("noti.site.id") + " " + siteId + newline + newline);
		// notification text
		buffer.append(rb.getFormattedMessage("noti.releasegrade.text",
				new String[] { a.getTitle(), siteTitle }));
	...
	private String htmlContentReleaseResubmission(AssignmentSubmission s)
	{
		...
		{
			M_log.warn(this + " htmlContentReleaseResubmission(), site id =" + context
					+ " " + ee.getMessage());
		}
		...
		buffer.append(rb.getString("noti.site.id") + " " + siteId + newline + newline);
		// notification text
		// Get the actual person that submitted, for a group submission just get
		// the first person from that group (This is why the array is used)
		String userId = null;
		...
		{
			buffer.append(rb.getFormattedMessage("noti.releaseresubmission.text",
					new String[] { a.getTitle(), siteTitle }));
		}
		else
		{
			buffer.append(
					rb.getFormattedMessage("noti.releaseresubmission.noresubmit.text",
							new String[] { a.getTitle(), siteTitle }));
		}
	...
	/**
	 * Cancel the changes made to a AssignmentSubmissionEdit object, and release
	 * the lock.
	 * 
	 * @param submission
	 *        The AssignmentSubmissionEdit object to commit.
	...
	public void cancelEdit(AssignmentSubmissionEdit submission)
	{
		...
		if (!submission.isActiveEdit())
		{
			...
			{
				M_log.warn(
						" cancelEdit(): closed AssignmentSubmissionEdit assignment submission id="
								+ submission.getId() + " " + e.getMessage());
			}
			return;
		}
			...
			throws PermissionException
	{
		if (submission != null)
		{
			if (!submission.isActiveEdit())
			{
				...
				{
					M_log.warn(" removeSubmission(): closed AssignmentSubmissionEdit id="
							+ submission.getId() + " " + e.getMessage());
				}
				return;
			}
			...
			// track event
			EventTrackingService.post(EventTrackingService.newEvent(
					AssignmentConstants.EVENT_REMOVE_ASSIGNMENT_SUBMISSION,
					submission.getReference(), true));
			...
			try
			{
				AuthzGroupService.removeAuthzGroup(
						AuthzGroupService.getAuthzGroup(submission.getReference()));
			}
			catch (AuthzPermissionException e)
			{
				M_log.warn(" removeSubmission: removing realm for : "
						+ submission.getReference() + " : " + e.getMessage());
			}
			catch (GroupNotDefinedException e)
			{
				M_log.warn(" removeSubmission: cannot find group for submission "
						+ submission.getReference() + " : " + e.getMessage());
			}
		}
	}// removeSubmission
	...
	/**
	 * Access all AssignmentSubmission objects - known to us (not from external
	 * providers).
	 * 
	...
	protected List getSubmissions(String context)
	{
		...
		if (contentReviewService != null)
		{
			try
			{
				List<ContentReviewItem> reports = contentReviewService.getReportList(null,
						context);
	...
	private void updateSubmissionList(List<AssignmentSubmission> submissions,
			List<ContentReviewItem> reports)
	{
		// lets build a map to avoid multiple searches through the list of
		// reports
	 ...
	 * @return Iterator over all the Assignments with the specified
	 *         AssignmentContent.
	 */
	 ...
	 *        - Describes the portlet context - generated with
	 *        DefaultId.getChannel().
	 * @return Iterator over all the Assignments associated with the context and
	 *         the user.
	 */
	 ...
	 * @inheritDoc
	 */
	public Map<Assignment, List<String>> getSubmittableAssignmentsForContext(
			String context)
	{
		...
		catch (IdUnusedException e)
		{
			...
			{
				M_log.debug(
						"Could not retrieve submittable assignments for nonexistent site: "
								+ context);
			}
		}
		if (site == null)
	...
	private Iterator assignmentsForContextAndUser(String context, String userId)
	{
		...
		if (context != null)
		{
			...
			for (int x = 0; x < allAssignments.size(); x++)
			{
				...
				if ((context.equals(tempAssignment.getContext())) || (context
						.equals(getGroupNameFromContext(tempAssignment.getContext()))))
				{
					retVal.add(tempAssignment);
				}
			}
		...
		if (context != null)
		{
			...
			for (int x = 0; x < allAssignments.size(); x++)
			{
				...
				if ((context.equals(tempAssignment.getContext())) || (context
						.equals(getGroupNameFromContext(tempAssignment.getContext()))))
				{
					String deleted = tempAssignment.getProperties()
							.getProperty(ResourceProperties.PROP_ASSIGNMENT_DELETED);
					if (deleted == null || "".equals(deleted))
					{
						// not deleted, show it
	...
	private boolean isDraftAssignmentVisible(Assignment assignment, String context)
	{
		return securityService.isSuperUser() // super user can always see it
				|| assignment.getCreator()
						.equals(UserDirectoryService.getCurrentUser().getId()) // the
																				// creator
																				// can
																				// see
																				// it
				|| (unlockCheck(SECURE_SHARE_DRAFTS, SiteService.siteReference(context))); // any
																							// role
																							// user
																							// with
																							// share
	...
	public AssignmentSubmission getSubmission(String assignmentReference, User person)
	{
		...
		if (submission != null)
		{
			try
			{
				unlock2(SECURE_ACCESS_ASSIGNMENT_SUBMISSION, SECURE_ACCESS_ASSIGNMENT,
						submission.getReference());
			}
			catch (PermissionException e)
			{
		...
		for (int z = 0; z < submissions.size(); z++)
		{
			...
			if (sub != null)
			{
				...
				for (int a = 0; a < submitters.size(); a++)
				{
					...
					M_log.debug(this + " getSubmission(List, User) comparing aUser id : "
							+ aUserId + " and chosen user id : " + person.getId());
					if (aUserId.equals(person.getId()))
					{

						M_log.debug(
								this + " getSubmission(List, User) found a match : return value is "
										+ sub.getId());
						retVal = sub;
					}
				}
	 ...
	 * @return The AssignmentSubmission corresponding to the id, or null if it
	 *         does not exist.
	 * @throws IdUnusedException
	 *         if there is no object with this id.
	 * @throws PermissionException
			...
			throws IdUnusedException, PermissionException
	{
		...
		// if current user is not the original submitter and if he doesn't have
		// grading permission, he should not have access to other people's
		// submission.
		String assignmentRef = assignmentReference(submission.getContext(),
				submission.getAssignmentId());
		if (!allowGradeSubmission(assignmentRef))
		{
			List submitterIds = submission.getSubmitterIds();
			if (submitterIds != null
					&& !submitterIds.contains(SessionManager.getCurrentSessionUserId()))
			{
				throw new PermissionException(SessionManager.getCurrentSessionUserId(),
						SECURE_ACCESS_ASSIGNMENT_SUBMISSION, submissionId);
			}
		}
		// track event
		// EventTrackingService.post(EventTrackingService.newEvent(AssignmentConstants.EVENT_ACCESS_ASSIGNMENT_SUBMISSION,
	...
	{
		// base the check for SECURE_ADD on the site, any of the site's groups,
		// and the channel
		// if the user can SECURE_ADD anywhere in that mix, they can add an
		// assignment
		// this stack is not the normal azg set for channels, so use a special
		// refernce to get this behavior
		String resourceString = getAccessPoint(true) + Entity.SEPARATOR
				+ REF_TYPE_ASSIGNMENT_GROUPS + Entity.SEPARATOR + "a" + Entity.SEPARATOR
				+ context + Entity.SEPARATOR;

		{
			M_log.debug(this + " allowAddGroupAssignment with resource string : "
					+ resourceString);
			M_log.debug(
	...
	public boolean allowReceiveSubmissionNotification(String context)
	{
		...
		{
			M_log.debug(
					this + " allowReceiveSubmissionNotification with resource string : "
							+ resourceString);
		}

		// checking allow at the site level
		if (unlockCheck(SECURE_ASSIGNMENT_RECEIVE_NOTIFICATIONS, resourceString))
			return true;
	...
	public List allowReceiveSubmissionNotificationUsers(String context)
	{
		...
		{
			M_log.debug(
					this + " allowReceiveSubmissionNotificationUsers with resource string : "
							+ resourceString);
			M_log.debug(
					"                                   				 	context string : "
							+ context);
		}
		return securityService.unlockUsers(SECURE_ASSIGNMENT_RECEIVE_NOTIFICATIONS,
				resourceString);
	...
	{
		String resourceString = getContextReference(context);
		// base the check for SECURE_ADD_ASSIGNMENT on the site and any of the
		// site's groups
		// if the user can SECURE_ADD_ASSIGNMENT anywhere in that mix, they can
		// add an assignment
		// this stack is not the normal azg set for site, so use a special
		// refernce to get this behavior

		{
			M_log.debug(this + " allowAddAssignment with resource string : "
					+ resourceString);
		}
	...
	public boolean allowAddSiteAssignment(String context)
	{
		...
		{
			M_log.debug(this + " allowAddSiteAssignment with resource string : "
					+ resourceString);
		}
	...
	public boolean allowAllGroups(String context)
	{
		...
		{
			M_log.debug(
					this + " allowAllGroups with resource string : " + resourceString);
		}
	 ...
	 * @inheritDoc
	 */
	public Collection getGroupsAllowGradeAssignment(String context,
			String assignmentReference)
	{
		...
		if (allowGradeSubmission(assignmentReference))
		{
			...
			Collection allAllowedGroups = getGroupsAllowFunction(
					SECURE_GRADE_ASSIGNMENT_SUBMISSION, context, null);
			try
			{
				Assignment a = getAssignment(assignmentReference);
				...
				else
				{
					...
					// for grouped assignment, return only those also allowed
					// for grading
			...
			{
				M_log.info(this + " getGroupsAllowGradeAssignment " + e.getMessage()
						+ assignmentReference);
			}
		}
	...
	public boolean allowGetAssignment(String context)
	{
		...
		{
			M_log.debug(this + " allowGetAssignment with resource string : "
					+ resourceString);
		}
	 ...
	 * @return True if the current User is allowed to update the Assignment,
	 *         false if not.
	 */
	public boolean allowUpdateAssignment(String assignmentReference)
	{
		M_log.debug(this + " allowUpdateAssignment with resource string : "
				+ assignmentReference);
	 ...
	 * 
	 * @return True if the current User is allowed to remove the Assignment,
	 *         false if not.
	 */
	...
	/**
	 * Get the groups of this channel's contex-site that the end user has
	 * permission to "function" in.
	 * 
	 * @param function
			...
			String userId)
	{
		...
		try
		{
			...
			// if the user has SECURE_ALL_GROUPS in the context (site), select
			// all site groups
			if (securityService.unlock(userId, SECURE_ALL_GROUPS,
					SiteService.siteReference(context))
			...
			groupRefs = AuthzGroupService.getAuthzGroupsIsAllowed(userId, function,
					groupRefs);

			// pick the Group objects from the site's groups to return, those
			// that are in the groupRefs list
			for (Iterator i = groups.iterator(); i.hasNext();)
			{
				Group group = (Group) i.next();
		...
		{
			M_log.debug(this + " getGroupsAllowFunction idunused :" + context + " : "
					+ e.getMessage());
		}
	 ...
	 * @return True if the current User is allowed to access the
	 *         AssignmentContent, false if not.
	 */
	public boolean allowGetAssignmentContent(String context)
	{
		String resourceString = getAccessPoint(true) + Entity.SEPARATOR + "c"
				+ Entity.SEPARATOR + context + Entity.SEPARATOR;

		{
			M_log.debug(this + " allowGetAssignmentContent with resource string : "
					+ resourceString);
		}
	 ...
	 * @param contentReference
	 *        - The AssignmentContent reference.
	 * @return True if the current User is allowed to update the
	 *         AssignmentContent, false if not.
	 */
	public boolean allowUpdateAssignmentContent(String contentReference)
	{

		M_log.debug(this + " allowUpdateAssignmentContent with resource string : "
				+ contentReference);
	 ...
	 *        - Describes the portlet context - generated with
	 *        DefaultId.getChannel().
	 * @return True if the current User is allowed to add an AssignmentContent,
	 *         false if not.
	 */
	public boolean allowAddAssignmentContent(String context)
	{
		String resourceString = getAccessPoint(true) + Entity.SEPARATOR + "c"
				+ Entity.SEPARATOR + context + Entity.SEPARATOR;
		M_log.debug(this + "allowAddAssignmentContent with resource string : "
				+ resourceString);
	 ...
	 * @param contentReference
	 *        - The AssignmentContent reference.
	 * @return True if the current User is allowed to remove the
	 *         AssignmentContent, false if not.
	 */
	public boolean allowRemoveAssignmentContent(String contentReference)
	{

		M_log.debug(this + " allowRemoveAssignmentContent with referece string : "
				+ contentReference);
	 ...
	 *        - Describes the portlet context - generated with
	 *        DefaultId.getChannel().
	 * @return True if the current User is allowed to add an
	 *         AssignmentSubmission, false if not.
	 */
	public boolean allowAddSubmission(String context)
	{
		// check security (throws if not permitted)
		String resourceString = getAccessPoint(true) + Entity.SEPARATOR + "s"
				+ Entity.SEPARATOR + context + Entity.SEPARATOR;

		M_log.debug(
				this + " allowAddSubmission with resource string : " + resourceString);
	 ...
	 * @param context
	 * @param assignment
	 *        - An Assignment object. Needed for the groups to be checked.
	 * @return
	 */
	public boolean allowAddSubmissionCheckGroups(String context, Assignment assignment)
	{
		...
		return unlockCheckWithGroups(SECURE_ADD_ASSIGNMENT_SUBMISSION, resourceString,
				assignment);
	}
	 ...
	 * @return the List (User) of users who can addSubmission() for this
	 *         assignment.
	 */
	public List allowAddSubmissionUsers(String assignmentReference)
	{
		return securityService.unlockUsers(SECURE_ADD_ASSIGNMENT_SUBMISSION,
				assignmentReference);
	 ...
	 * @param assignmentReference
	 *        - a reference to an assignment
	 * @return the List (User) of users who can grade submission for this
	 *         assignment.
	 */
	public List allowGradeAssignmentUsers(String assignmentReference)
	{
		List users = securityService.unlockUsers(SECURE_GRADE_ASSIGNMENT_SUBMISSION,
				assignmentReference);
		if (users == null)
		{
			users = new ArrayList();
		...
		try
		{
			...
			{
				// for grouped assignment, need to include those users that with
				// "all.groups" and "grade assignment" permissions on the site
				// level
				AuthzGroup group = AuthzGroupService
						.getAuthzGroup(SiteService.siteReference(a.getContext()));
				if (group != null)
				{
					// get the roles which are allowed for submission but not
					// for all_site control
					Set rolesAllowAllSite = group.getRolesIsAllowed(SECURE_ALL_GROUPS);
					Set rolesAllowGradeAssignment = group
							.getRolesIsAllowed(SECURE_GRADE_ASSIGNMENT_SUBMISSION);
					// save all the roles with both "all.groups" and "grade
					...
					{
						for (Iterator iRoles = rolesAllowAllSite.iterator(); iRoles
								.hasNext();)
						{
							Set<String> userIds = group
									.getUsersHasRole((String) iRoles.next());
							if (userIds != null)
							{
								for (Iterator<String> iUserIds = userIds
										.iterator(); iUserIds.hasNext();)
								{
									...
									{
										M_log.warn(" allowGradeAssignmentUsers "
												+ ee.getMessage()
		...
		{
			M_log.warn(" allowGradeAssignmentUsers " + e.getMessage()
	...
	public List allowAddAnySubmissionUsers(String context)
	{
		...
		{
			AuthzGroup group = AuthzGroupService
					.getAuthzGroup(SiteService.siteReference(context));

			// get the roles which are allowed for submission but not for
			// all_site control
			Set rolesAllowSubmission = group
					.getRolesIsAllowed(SECURE_ADD_ASSIGNMENT_SUBMISSION);
			Set rolesAllowAllSite = group.getRolesIsAllowed(SECURE_ALL_GROUPS);
		...
		{
			M_log.warn(" allowAddAnySubmissionUsers " + e.getMessage() + " context="
					+ context);
		}
	...
	public List allowAddAssignmentUsers(String context)
	{
		...
		{
			M_log.debug(this + " allowAddAssignmentUsers with resource string : "
					+ resourceString);
			M_log.debug(
					"                                    	context string : " + context);
		}
	 ...
	 * @return True if the current User is allowed to get the
	 *         AssignmentSubmission, false if not.
	 */
	public boolean allowGetSubmission(String submissionReference)
	{
		M_log.debug(this + " allowGetSubmission with resource string : "
				+ submissionReference);

		return unlockCheck2(SECURE_ACCESS_ASSIGNMENT_SUBMISSION, SECURE_ACCESS_ASSIGNMENT,
				submissionReference);
	}
	...
	{
		M_log.debug(this + " allowUpdateSubmission with resource string : "
				+ submissionReference);

		return unlockCheck2(SECURE_UPDATE_ASSIGNMENT_SUBMISSION, SECURE_UPDATE_ASSIGNMENT,
				submissionReference);
	}
	...
	{
		M_log.debug(this + " allowRemoveSubmission with resource string : "
				+ submissionReference);
	...
	public boolean allowGradeSubmission(String assignmentReference)
	{
		...
		{
			M_log.debug(this + " allowGradeSubmission with resource string : "
					+ assignmentReference);
		}
	...
	/**
	 * Access the grades spreadsheet for the reference, either for an assignment
	 * or all assignments in a context.
	 * 
	 * @param ref
	 *        The reference, either to a specific assignment, or just to an
	 *        assignment context.
	...
	/**
	 * Access and output the grades spreadsheet for the reference, either for an
	 * assignment or all assignments in a context.
	 *
	 * @param out
			...
			throws IdUnusedException, PermissionException
	{
		...
		String context = ref
				.substring(ref.indexOf(typeGradesString) + typeGradesString.length());
		...
		{
			// ignore exception
			M_log.debug(this + ":getGradesSpreadsheet cannot get site context=" + context
					+ e.getMessage());
		}
		...
		for (int iAssignment = 0; !allowGradeAny
				&& iAssignment < assignmentsList.size(); iAssignment++)
		{
			if (allowGradeSubmission(
					((Assignment) assignmentsList.get(iAssignment)).getReference()))
			{
				allowGradeAny = true;
			}
		}
		...
		else
		{
			...
			row = sheet.createRow(rowNum++);
			row.createCell(0)
					.setCellValue(rb.getString("download.spreadsheet.site") + siteTitle);

			// download time
			row = sheet.createRow(rowNum++);
			row.createCell(0).setCellValue(rb.getString("download.spreadsheet.date")
			...
			for (Iterator iUserIds = new SortedIterator(
					allowAddAnySubmissionUsers.iterator(),
					new AssignmentComparator("sortname", "true")); iUserIds.hasNext();)
			{
				...
				{
					M_log.warn(" getGradesSpreadSheet " + e.getMessage() + " userId = "
							+ userId);
				}
			}

			int index = 0;
			// the grade data portion starts from the third column, since the
			// first two are used for user's display id and sort name
			while (assignments.hasNext())
			{
				...
				// for column header, check allow grade permission based on each
				// assignment
				if (!a.getDraft() && allowGradeSubmission(a.getReference()))
				{
					...
					cellNum = (index + 2);
					cell = row.createCell(cellNum); // since the first two
													// column is taken by
													// student id and name
					cell.setCellStyle(style);
					...
					{
						// prepopulate the column with the "no submission"
						// string
						row = sheet.getRow(rowNum++);
						cell = row.createCell(cellNum);
						cell.setCellType(1);
					...
					// begin to populate the column for this assignment,
					// iterating through student list
					for (Iterator sIterator = getSubmissions(a).iterator(); sIterator
							.hasNext();)
					{
						AssignmentSubmission submission = (AssignmentSubmission) sIterator
								.next();
						...
						if (a.isGroup())
						{
							...
							for (int i = 0; _users != null && i < _users.length; i++)
							{
								...
								if (user_row.containsKey(userId))
								{
									// find right row
									row = sheet.getRow(
											((Integer) user_row.get(userId)).intValue());
											...
											&& submission.getGrade() != null)
									{
										...
										if (assignmentType == 3)
										{
											...
											{
												// numeric cell type?
												String grade = submission
														.getGradeForUser(userId) == null
																? submission
																		.getGradeDisplay()
																: submission
																		.getGradeForUser(
												...
												// We get float number no matter
												// the locale it was managed
												// with.
												NumberFormat nbFormat = FormattedText
														.getNumberFormat(1, 1, null);
												float f = nbFormat.parse(grade)
														.floatValue();

												// remove the String-based cell
												// first
											...
											catch (Exception e)
											{
												...
												// let's make it as String type
												// No need to remove the cell
												// and create a new one, as the
												// existing one is String type.
												cell = row.getCell(cellNum);
												cell.setCellType(1);
												cell.setCellValue(submission
														.getGradeForUser(userId) == null
																? submission
																		.getGradeDisplay()
																: submission
										...
										else
										{
											...
											cell.setCellValue(submission
													.getGradeForUser(userId) == null
															...
															: submission.getGradeForUser(
																	userId));
										}
									}
									else if (submission.getSubmitted()
										...
										&& submission.getGrade() != null)
								{
									...
									if (assignmentType == 3)
									{
										try
										{
											...
											// We get float number no matter the
											// locale it was managed with.
											NumberFormat nbFormat = FormattedText
													.getNumberFormat(1, 1, null);
										...
										catch (Exception e)
										{
											// if the grade is not numeric,
											// let's make it as String type
											// No need to remove the cell and
											// create a new one, as the existing
											// one is String type.
											cell = row.getCell(cellNum);
											cell.setCellType(1);
											// Setting grade display instead
											// grade.
											cell.setCellValue(
													submission.getGradeDisplay());
										}
			...
			{
				M_log.warn(
						" getGradesSpreadsheet Can not output the grade spread sheet for reference= "
								+ ref);
			}
	...
	public Collection<Group> getSubmitterGroupList(String searchFilterOnly,
			String allOrOneGroup, String searchString, String aRef, String contextString)
	{
			...
			if (a != null)
			{
				...
				if (a.getAccess().equals(Assignment.AssignmentAccess.SITE))
				{
					...
					for (Iterator gIterator = groupRefs.iterator(); gIterator.hasNext();)
					{
						...
						// if (_gg.getProperties().get(GROUP_SECTION_PROPERTY)
						// == null) { // NO SECTIONS (this might not be valid
						// test for manually created sections)
						rv.add(_gg);
						// }
					}
				}
				else
				{
					...
					for (Iterator gIterator = groupRefs.iterator(); gIterator.hasNext();)
					{
						Group _gg = st.getGroup((String) gIterator.next()); // NO
																			// SECTIONS
																			// (this
																			// might
																			// not
						...
						if (_gg != null)
						{// &&
							// _gg.getProperties().get(GROUP_SECTION_PROPERTY)
							// == null) {
							rv.add(_gg);
						}
					}
				...
				for (Iterator uIterator = rv.iterator(); uIterator.hasNext();)
				{
					...
					if (uSubmission == null)
					{
						if (allowGradeSubmission(a.getReference()))
						{
							if (a.isGroup())
							{
								// temporarily allow the user to read and write
								// from assignments (asn.revise permission)
								SecurityAdvisor securityAdvisor = new MySecurityAdvisor(
										...
										""/*
											 * no submission id yet, pass the
											 * empty string to advisor
											 */);
								try
								{
									securityService.pushAdvisor(securityAdvisor);

									M_log.debug(this + " getSubmitterGroupList context "
											+ contextString + " for assignment "
									...
									// get the assignment setting for
									// resubmitting
									ResourceProperties assignmentProperties = a
											.getProperties();
									...
									{
										s.getPropertiesEdit().addProperty(
												AssignmentSubmission.ALLOW_RESUBMIT_NUMBER,
										...
										// if assignment's setting of resubmit
										// close time is null, use assignment
										// close time as the close time for
										// resubmit
										s.getPropertiesEdit()
												.addProperty(
														AssignmentSubmission.ALLOW_RESUBMIT_CLOSETIME,
		...
		{
			M_log.warn(":getSubmitterGroupList: Assignme id not used: " + aRef + " "
					+ aIdException.getMessage());
		}

		catch (PermissionException aPerException)
		{
			M_log.warn(":getSubmitterGroupList: Not allowed to get assignment " + aRef
					+ " " + aPerException.getMessage());
		}
	 ...
	 * {@inheritDoc}}
	 */
	public List<String> getSubmitterIdList(String searchFilterOnly, String allOrOneGroup,
			String searchString, String aRef, String contextString)
	{

		List<String> rv = new ArrayList<String>();
		Map<User, AssignmentSubmission> submitterMap = getSubmitterMap(searchFilterOnly,
				allOrOneGroup, searchString, aRef, contextString);
	...
	// alternative to getSubmittedIdList which returns full user and
	// submissions, since submitterIdList retrieves them anyway
	public Map<User, AssignmentSubmission> getSubmitterMap(String searchFilterOnly,
			String allOrOneGroup, String searchString, String aRef, String contextString)
	{
		...
		try
		{
			...
			if (bSearchFilterOnly)
			{
				...
				{
					// if the option is set to "Only show user submissions
					// according to Group Filter and Search result"
					// if no group filter and no search string is specified, no
					// user will be shown first by default;
					return rv;
				}
				else
				{
					...
					if (allOrOneGroup == null && searchString != null)
					{
						...
						rvUsers = getSearchedUsers(searchString, allowAddSubmissionUsers,
								false);
					}
					else
					{
						// group filter first
						rvUsers = getSelectedGroupUsers(allOrOneGroup, contextString, a,
								allowAddSubmissionUsers);
			...
			else
			{
				...
				// SAK-28055 need to take away those users who have the
				// permissions defined in sakai.properties
				String resourceString = getContextReference(a.getContext());
				String[] permissions = m_serverConfigurationService
						.getStrings("assignment.submitter.remove.permission");
				if (permissions != null)
				{
					for (String permission : permissions)
					{
						allowAddSubmissionUsers.removeAll(
								securityService.unlockUsers(permission, resourceString));
					}
				}
				else
				{
					allowAddSubmissionUsers.removeAll(securityService
							.unlockUsers(SECURE_ADD_ASSIGNMENT, resourceString));
				}
				...
				// Step 2: get all student that meets the search criteria based
				// on previous group users. If search is null or empty string,
				// return all users.
				rvUsers = getSearchedUsers(searchString, rvUsers, true);
			}
			...
			if (!rvUsers.isEmpty())
			{
				...
				for (Iterator uIterator = rvUsers.iterator(); uIterator.hasNext();)
				{
					...
					// add those users who haven't made any submissions and with
					// submission rights
					else
					{
						...
						if (groupRefs.isEmpty())
						{
							if (a.getAccess() == Assignment.AssignmentAccess.SITE)
							{
								// for site range assignment, add the site
								// reference first
								groupRefs.add(SiteService.siteReference(contextString));
							}
							// add all groups inside the site
							Collection groups = getGroupsAllowGradeAssignment(
									contextString, a.getReference());
							for (Object g : groups)
							{
						...
						// construct fake submissions for grading purpose if the
						// user has right for grading
						if (allowGradeSubmission(a.getReference()))
						{
							SecurityAdvisor securityAdvisor = new MySecurityAdvisor(
									SessionManager.getCurrentSessionUserId(),
									new ArrayList<String>(Arrays.asList(
											...
											SECURE_UPDATE_ASSIGNMENT_SUBMISSION)),
									groupRefs/*
												 * no submission id yet, pass
												 * the empty string to advisor
												 */);
							try
							{
								// temporarily allow the user to read and write
								...
								AssignmentSubmissionEdit s = addSubmission(contextString,
										a.getId(), u.getId());
								if (s != null)
								{
									// Note: If we had s.setSubmitted(false);,
									// this would put it in 'draft mode'
									s.setSubmitted(true);
									/*
									 * SAK-29314 - Since setSubmitted represents
									 * whether the submission is in draft mode
									 * state, we need another property. So we
									 * created isUserSubmission. This represents
									 * whether the submission was geenrated by a
									 * user. We set it to false because these
									 * submissions are generated so that the
									 * instructor has something to grade; the
									 * user did not in fact submit anything.
									 */
		...
		{
			M_log.warn(":getSubmitterIdList: Assignme id not used: " + aRef + " "
					+ aIdException.getMessage());
		}

		catch (PermissionException aPerException)
		{
			M_log.warn(":getSubmitterIdList: Not allowed to get assignment " + aRef + " "
					+ aPerException.getMessage());
		}
	...
	private List<User> getSelectedGroupUsers(String allOrOneGroup, String contextString,
			Assignment a, List allowAddSubmissionUsers)
	{
		...
		if (allOrOneGroup != null && allOrOneGroup.length() > 0)
		{
			...
			if (allOrOneGroup.equals(AssignmentConstants.ALL))
			{
				if (allowAllGroups(contextString))
				{
					...
					{
						M_log.warn(":getSelectedGroupUsers cannot find site " + " "
								+ contextString + e.getMessage());
					}
				}
				else
				{
					// get all those groups that user is allowed to grade
					groups = getGroupsAllowGradeAssignment(contextString,
							a.getReference());
				}
			}
			else
			{
				// filter out only those submissions from the selected-group
				// members
				try
				{
					Group group = SiteService.getSite(contextString)
							.getGroup(allOrOneGroup);
					groups.add(group);
				}
				catch (Exception e)
				{
					M_log.warn(":getSelectedGroupUsers " + e.getMessage() + " groupId="
							+ allOrOneGroup);
				}
			}
			...
			for (Iterator iGroup = groups.iterator(); iGroup.hasNext();)
			{
				...
				String authzGroupRef = (nGroup instanceof Group)
						? ((Group) nGroup).getReference()
						: ((nGroup instanceof Site)) ? ((Site) nGroup).getReference()
								: null;
				if (authzGroupRef != null)
				{
					try
					{
						...
						for (Iterator iUserIds = grants.iterator(); iUserIds.hasNext();)
						{
							...
							{
								M_log.warn(":getSelectedGroupUsers "
										+ uException.getMessage() + " userId =" + userId);
							}
						}
					}
					catch (GroupNotDefinedException gException)
					{
						M_log.warn(":getSelectedGroupUsers " + gException.getMessage()
								+ " authGroupId=" + authzGroupRef);
					}
				}
			}
		}
	 ...
	 * @param userList
	 * @param retain
	 *        If true, the original list will be kept if there is no search
	 *        string specified
	 * @return
	 */
	 ...
	 * {@inheritDoc}
	 */
	public void getSubmissionsZip(OutputStream outputStream, String ref)
			throws IdUnusedException, PermissionException
	{
	...
	/**
	 * depends on the query string from ui, determine what to include inside the
	 * submission zip
	 * 
	 * @param outputStream
	 * @param ref
	 ...
	 * @throws PermissionException
	 */
	protected void getSubmissionsZip(OutputStream out, String ref, String queryString)
			throws IdUnusedException, PermissionException
	{
		...
		try
		{
			...
			if (a.isGroup())
			{
				Collection<Group> submitterGroups = getSubmitterGroupList(
						...
						searchString, aRef,
						contextString == null ? a.getContext() : contextString);
				if (submitterGroups != null && !submitterGroups.isEmpty())
				{
							...
							.iterator(); iSubmitterGroupsIterator.hasNext();)
					{
						...
						M_log.debug(this + " ZIP GROUP " + g.getTitle() + " SUB "
					...
					{
						zipGroupSubmissions(aRef, a.getTitle(),
								a.getContent().getTypeOfGradeString(
										a.getContent().getTypeOfGrade()),
								a.getContent().getTypeOfSubmission(),
								new SortedIterator(submissions.iterator(),
										new AssignmentComparator("submitterName",
												"true")),
								out, exceptionMessage, withStudentSubmissionText,
								withStudentSubmissionAttachment, withGradeFile,
								withFeedbackText, withFeedbackComment,
								withFeedbackAttachment, gradeFileFormat,
								includeNotSubmitted);
						...
						if (exceptionMessage.length() > 0)
						{
							...
							M_log.warn(" getSubmissionsZip ref=" + ref
			...
			else
			{
				...
				// getSubmitterIdList(searchFilterOnly, viewString.length() == 0
				// ? AssignmentConstants.ALL:viewString, searchString, aRef,
				// contextString == null? a.getContext():contextString);
				...
				if (!submitters.isEmpty())
				{
					...
					if (allowGradeSubmission(aRef))
					{
						...
						zipSubmissions(aRef, a.getTitle(),
								content.getTypeOfGradeString(content.getTypeOfGrade()),
								content.getTypeOfSubmission(),
								new SortedIterator(submissions.iterator(),
										new AssignmentComparator("submitterName",
												"true")),
								out, exceptionMessage, withStudentSubmissionText,
								withStudentSubmissionAttachment, withGradeFile,
								withFeedbackText, withFeedbackComment,
								withFeedbackAttachment, withoutFolders, gradeFileFormat,
								includeNotSubmitted);
		...
		catch (IdUnusedException e)
		{
			...
			M_log.debug(
					this + "getSubmissionsZip -IdUnusedException Unable to get assignment "
							+ ref);
		...
		{
			M_log.warn(
					" getSubmissionsZip -PermissionException Not permitted to get assignment "
							+ ref);
			throw new PermissionException(SessionManager.getCurrentSessionUserId(),
					SECURE_ACCESS_ASSIGNMENT, ref);
		}
	...
	protected void zipGroupSubmissions(String assignmentReference, String assignmentTitle,
			String gradeTypeString, int typeOfSubmission, Iterator submissions,
			OutputStream outputStream, StringBuilder exceptionMessage,
			boolean withStudentSubmissionText, boolean withStudentSubmissionAttachment,
			boolean withGradeFile, boolean withFeedbackText, boolean withFeedbackComment,
			boolean withFeedbackAttachment, String gradeFileFormat,
			boolean includeNotSubmitted)
	{
		...
		try
		{
			...
			String root = escapeInvalidCharsEntry(
					Validator.escapeZipEntry(assignmentTitle)) + Entity.SEPARATOR;

			SpreadsheetExporter.Type type = SpreadsheetExporter.Type
					.valueOf(gradeFileFormat.toUpperCase());
			SpreadsheetExporter sheet = SpreadsheetExporter.getInstance(type,
					assignmentTitle, gradeTypeString);
			...
			// Write the header
			sheet.addHeader("Group", rb.getString("grades.eid"),
					rb.getString("grades.members"), rb.getString("grades.grade"));
			...
			while (submissions.hasNext())
			{
				...
				// SAK-29314 added a new value where it's by default submitted
				// but is marked when the user submits
				if ((s.getSubmitted() && s.isUserSubmission()) || includeNotSubmitted)
				{
					try
					{
						...
						User[] submitters = s.getSubmitters();
						String submitterString = gs.getGroup().getTitle() + " ("
								+ gs.getGroup().getId() + ")";
						...
						for (int i = 0; i < submitters.length; i++)
						{
							...
							// in case the user doesn't have first name or last
							// name
							...
							submitters2String = submitters2String
									.concat(submitters[i].getDisplayName());
							// add the eid to the end of it to guarantee folder
							// name uniqness
							submittersString = submittersString + "("
									+ submitters[i].getEid() + ")";
						}

						// Adding the row
						sheet.addRow(gs.getGroup().getTitle(), gs.getGroup().getId(),
								submitters2String, s.getGradeDisplay());

						if (StringUtil.trimToNull(submitterString) != null)
						{
							submittersName = submittersName
									.concat(StringUtil.trimToNull(submitterString));
									...
									&& typeOfSubmission != Assignment.NON_ELECTRONIC_ASSIGNMENT_SUBMISSION)
							{
								...
								if (withStudentSubmissionText)
								{
									...
									ZipEntry textEntry = new ZipEntry(submittersName
											+ submitterString + "_submissionText"
								...
								if (withFeedbackText)
								{
									...
									ZipEntry fTextEntry = new ZipEntry(
											submittersName + "feedbackText.html");
									...
									&& typeOfSubmission != Assignment.NON_ELECTRONIC_ASSIGNMENT_SUBMISSION)
							{
								...
								{
									// create a attachment folder for the
							...
							if (withFeedbackComment)
							{
								...
								out.putNextEntry(textEntry);
								byte[] b = FormattedText
										.encodeUnicode(s.getFeedbackComment()).getBytes();
								out.write(b);
								textEntry.setSize(b.length);
								out.closeEntry();
							}
							...
							if (submittersString.trim().length() > 0)
							{
								...
								out.putNextEntry(textEntry);
								byte[] b = FormattedText.encodeUnicode(submittersString)
										.getBytes();
								out.write(b);
								textEntry.setSize(b.length);
								out.closeEntry();
			...
			if (caughtException == null)
			{
				...
				if (withGradeFile)
				{
					ZipEntry gradesCSVEntry = new ZipEntry(
							root + "grades." + sheet.getFileExtension());
			...
			else
			{
				...
				exceptionMessage.append(" Exception " + caughtException
						+ " for creating submission zip file for assignment " + "\""
						+ assignmentTitle + "\"\n");
			}
		}
		catch (IOException e)
		{
			exceptionMessage
					.append("IOException for creating submission zip file for assignment "
	...
	protected void zipSubmissions(String assignmentReference, String assignmentTitle,
			String gradeTypeString, int typeOfSubmission, Iterator submissions,
			OutputStream outputStream, StringBuilder exceptionMessage,
			boolean withStudentSubmissionText, boolean withStudentSubmissionAttachment,
			boolean withGradeFile, boolean withFeedbackText, boolean withFeedbackComment,
			boolean withFeedbackAttachment, boolean withoutFolders,
			String gradeFileFormat, boolean includeNotSubmitted)
	{
		...
		try
		{
			...
			sheet.addHeader(rb.getString("grades.id"), rb.getString("grades.eid"),
			...
			while (submissions.hasNext())
			{
				...
				if ((s.getSubmitted() && s.isUserSubmission()) || includeNotSubmitted)
				{
					// get the submission user id and see if the user is still
					// in site
					...
					try
					{
						...
						if (allowAddSubmissionUsers.contains(u))
						{
							...
							for (int i = 0; i < submitters.length; i++)
							{
								...
								String fullName = submitters[i].getSortName();
								// in case the user doesn't have first name or
								// last name
								if (fullName.indexOf(",") == -1)
								{
									fullName = fullName.concat(",");
								...
								submittersString = submittersString.concat(fullName);
								// add the eid to the end of it to guarantee
								// folder name uniqness
								// if user Eid contains non ascii characters,
								// the user internal id will be used
								...
								submittersString = escapeInvalidCharsEntry(
										submittersString);

								String fullAnonId = s.getAnonymousSubmissionId();
								String anonTitle = rb
										.getString("grading.anonymous.title");

								// SAK-17606
								if (!isAnon)
								{
									sheet.addRow(submitters[i].getDisplayId(),
											submitters[i].getEid(),
											submitters[i].getLastName(),
											submitters[i].getFirstName(),
											s.getGradeDisplay());
								}
								else
								{
									sheet.addRow(fullAnonId, fullAnonId, anonTitle,
											anonTitle, s.getGradeDisplay());
								}
							}
							...
							{
								submittersName = submittersName
										.concat(StringUtils.trimToNull(submittersString));
								submittedText = s.getSubmittedText();
								...
								if (!withoutFolders)
								{
									if (s.getSubmitted() && s.getTimeSubmitted() != null)
									{
										ZipEntry textEntry = new ZipEntry(
												submittersName + "timestamp.txt");
										out.putNextEntry(textEntry);
										byte[] b = (s.getTimeSubmitted().toString())
												.getBytes();
										out.write(b);
										textEntry.setSize(b.length);
										out.closeEntry();
										...
										&& typeOfSubmission != Assignment.NON_ELECTRONIC_ASSIGNMENT_SUBMISSION)
								{
									...
									if (withStudentSubmissionText)
									{
										// create the text file only when a text
										// submission is allowed
										String submittersNameString = submittersName
												+ submittersString;

										// remove folder name if Download All is
										// without user folders
										if (withoutFolders)
										{
											submittersNameString = submittersName;
										}
										ZipEntry textEntry = new ZipEntry(
												submittersNameString + "_submissionText"
														+ ZIP_SUBMITTED_TEXT_FILE_TYPE);
										out.putNextEntry(textEntry);
										byte[] text = submittedText.getBytes();
										out.write(text);
										...
										&& typeOfSubmission != Assignment.NON_ELECTRONIC_ASSIGNMENT_SUBMISSION)
								{
									...
									if (withStudentSubmissionAttachment)
									{
										// remove "/" that creates a folder if
										// Download All is without user folders
										String sSubAttachmentFolder = submittersName
										...
										{
											sSubAttachmentFolder = sSubAttachmentFolder
													+ "_";
											// submittersName =
											// submittersName.concat("_");
										}

										// add all submission attachment into
										// the submission attachment folder
										zipAttachments(out, submittersName,
												sSubAttachmentFolder,
												s.getSubmittedAttachments());
										out.closeEntry();
								...
								if (withFeedbackAttachment)
								{
									// create an attachment folder for the
									...
									if (!withoutFolders)
									{
										...
										ZipEntry feedbackSubAttachmentFolderEntry = new ZipEntry(
												feedbackSubAttachmentFolder);
										out.putNextEntry(
												feedbackSubAttachmentFolderEntry);
									}
									else
									{
										submittersName = submittersName.concat("_");
	...
	/*
	 * SAK-17606 - If the assignment uses anonymous grading returns true, else
	 * false Params: AssignmentSubmission s
	 */
	...
	public boolean assignmentUsesAnonymousGrading(Assignment a)
	{
		...
		{
			M_log.warn("Entity Property " + NEW_ASSIGNMENT_CHECK_ANONYMOUS_GRADING
					+ " not defined " + e.getMessage());
		}
		catch (EntityPropertyTypeException e)
		{
			M_log.warn("Entity Property " + NEW_ASSIGNMENT_CHECK_ANONYMOUS_GRADING
					+ " type not defined " + e.getMessage());
		}
		return false;
	}

	private void zipAttachments(ZipOutputStream out, String submittersName,
			String sSubAttachmentFolder, List attachments)
	{
		...
		for (int j = 0; j < attachments.size(); j++)
		{
			...
			try
			{
				...
				String displayName = props
						.getPropertyFormatted(props.getNamePropDisplayName());
				...
				try
				{
					...
					{
						M_log.warn(":zipAttachments: problem closing zip entry "
								+ ioException);
					}
				}
				...
				{
					M_log.warn(
							":zipAttachments: problem creating BufferedInputStream with content and length "
				...
				finally
				{
					if (bContent != null)
					{
						...
						{
							M_log.warn(":zipAttachments: problem closing FileChannel "
									+ ioException);
						}
					}
				}
			...
			{
				M_log.warn(
						" zipAttachments--IOException: Problem in creating the attachment file: submittersName="
								+ submittersName + " attachment reference=" + r
								+ " error " + e);
			}
			...
			finally
			{
				if (content != null)
				{
					...
					{
						M_log.warn(":zipAttachments: problem closing Inputstream content "
								+ ioException);
					}
				}
			}
	 ...
	 * @param assignmentId
	 *        The id for the assignment object; when null, indicates all
	...
	public String gradesSpreadsheetReference(String context, String assignmentId)
	{
		...
		String s = REFERENCE_ROOT + Entity.SEPARATOR + REF_TYPE_GRADES + Entity.SEPARATOR
				+ context;
	...
	public String submissionsZipReference(String context, String assignmentReference)
	{
		...
		return REFERENCE_ROOT + Entity.SEPARATOR + REF_TYPE_SUBMISSIONS + Entity.SEPARATOR
				+ context + Entity.SEPARATOR + assignmentReference;

	} // submissionsZipReference

	/**
	 * Decode the submissionsZipReference string to get the assignment reference
	 * String
	 * 
	 * @param sReference
	...
	private String assignmentReferenceFromSubmissionsZipReference(String sReference)
	{
		...
		{
			return sReference
					.substring(sReference.lastIndexOf(Entity.SEPARATOR + "assignment"));
		}
		else
		{
			return sReference.substring(
					sReference.lastIndexOf(Entity.SEPARATOR + "assignment"),
					sReference.indexOf(Entity.SEPARATOR + "site"));
		}

	} // assignmentReferenceFromSubmissionsZipReference

	/**
	 * Decode the submissionsZipReference string to get the group reference
	 * String
	 * 
	 * @param sReference
	 *        The submissionZipReference String
	...
	private String groupReferenceFromSubmissionsZipReference(String sReference)
	{
		...
		{
			return sReference
					.substring(sReference.lastIndexOf(Entity.SEPARATOR + "site"));
		}
		else
		{
			return null;
		...
		return new HttpAccess()
		{
					...
					EntityAccessOverloadException, EntityCopyrightException
			{
				...
				else
				{
					try
					{
						if (REF_TYPE_SUBMISSIONS.equals(ref.getSubType()))
						{
							...
							res.setHeader("Content-Disposition",
									"attachment; filename = bulk_download.zip");
							...
							{
								M_log.error(this + " getHttpAccess handleAccess "
										+ ignore.getMessage() + " ref="
										+ ref.getReference());
							}
							finally
							{
						...
						else if (REF_TYPE_GRADES.equals(ref.getSubType()))
						{
							...
							res.setHeader("Content-Disposition",
									"attachment; filename = export_grades_file.xls");
						...
						{
							M_log.warn("handleAccess: throw IdUnusedException "
									+ ref.getReference());
					...
					{
						M_log.warn(" HandleAccess: caught exception " + t.toString()
								+ " and rethrow it!");
	...
	public boolean parseEntityReference(String reference, Reference ref)
	{
		if (reference.startsWith(REFERENCE_ROOT))
		{
			...
			// Note: StringUtils.split would not produce the following first
			// null part
			// Still use StringUtil here.
			String[] parts = StringUtil.split(reference, Entity.SEPARATOR);
			// we will get null, assignment, [a|c|s|grades|submissions],
			// context, [auid], id
	...
	public Entity getEntity(Reference ref)
	{
		...
		try
		{
			...
			else if (REF_TYPE_SUBMISSION.equals(ref.getSubType()))
			{
				...
				M_log.warn("getEntity(): unknown message ref subtype: " + ref.getSubType()
	...
	public Collection getEntityAuthzGroups(Reference ref, String userId)
	{
		...
		// if access set to GROUPED, use the assignment, and the groups, but not
		// the site authzGroups.
		// if the user has SECURE_ALL_GROUPS in the context, ignore GROUPED
		// access and treat as if SITE
		...
		try
		{
			...
			if (REF_TYPE_ASSIGNMENT.equals(ref.getSubType()))
			{
				...
				// check SECURE_ALL_GROUPS - if not, check if the assignment has
				// groups or not
				// TODO: the last param needs to be a
				// ContextService.getRef(ref.getContext())... or a
				// ref.getContextAuthzGroup() -ggolden
				if ((userId == null) || ((!securityService.isSuperUser(userId))
						&& (!securityService.unlock(userId, SECURE_ALL_GROUPS,
								SiteService.siteReference(ref.getContext())))))
				{
					// get the channel to get the message to get group
					// information
					// TODO: check for efficiency, cache and thread local
					// caching usage -ggolden
					if (ref.getId() != null)
					{
						Assignment a = findAssignment(ref.getReference());
						if (a != null)
						{
							grouped = Assignment.AssignmentAccess.GROUPED == a
									.getAccess();
							groups = a.getGroups();
						}
					}
		...
		{
			M_log.warn(" getEntityAuthzGroups(): " + e.getMessage() + " ref="
					+ ref.getReference());
		}
	...
	public String getEntityUrl(Reference ref)
	{
		...
		try
		{
			...
			else if (REF_TYPE_SUBMISSION.equals(ref.getSubType()))
			{
				...
				M_log.warn(" getEntityUrl(): unknown message ref subtype: "
						+ ref.getSubType() + " in ref: " + ref.getReference());
		}
		catch (PermissionException e)
		{
			...
			List attachments)
	{
		...
		// String assignRef = assignmentReference(siteId,
		// SiteService.MAIN_CONTAINER);
		...
		while (assignmentsIterator.hasNext())
		{
			...
			if (content != null)
			{
				...
				for (int i = 0; i < atts.size(); i++)
				{
					...
					// if it's in the attachment area, and not already in the
					// list
					...
					// in order to make assignment.xml has the consistent format
					// with the other xml files
					// move the attachments to be the children of the content,
				...
				{
					AssignmentSubmission submission = (AssignmentSubmission) submissionsIterator
							.next();
			...
			Set userListAllowImport)
	{
			...
			for (int i2 = 0; i2 < length2; i2++)
			{
				...
				if (child2.getNodeType() == Node.ELEMENT_NODE)
				{
					...
					if (element2.getTagName().equals("assignment"))
					{
						...
						// traverse this assignment node first to check if the
						// person who last modified, has the right role.
						...
						for (int i3 = 0; i3 < length3; i3++)
						{
							...
							if (child3.getNodeType() == Node.ELEMENT_NODE)
							{
								...
								// add the properties childnode to the clone of
								// assignment node
								if (element3.getTagName().equals("properties"))
								{
									...
									for (int i6 = 0; i6 < length6; i6++)
									{
										...
										if (child6.getNodeType() == Node.ELEMENT_NODE)
										{
											...
											{
												if (element6.getAttribute("name")
														.equalsIgnoreCase(
																"CHEF:modifiedby"))
												{
													if ("BASE64".equalsIgnoreCase(
															element6.getAttribute("enc")))
													{
														String creatorId = Xml
																.decodeAttribute(element6,
																		"value");
														if (!userListAllowImport
																.contains(creatorId))
															goAhead = false;
													}
													else
													{
														String creatorId = element6
																.getAttribute("value");
														if (!userListAllowImport
																.contains(creatorId))
															goAhead = false;
						...
						if (goAhead)
						{
							for (int i3 = 0; i3 < length3; i3++)
							{
								...
								if (child3.getNodeType() == Node.ELEMENT_NODE)
								{
									...
									// add the properties childnode to the clone
									// of assignment node
									if (element3.getTagName().equals("properties"))
									{
										// add the properties childnode to the
										// clone of assignment node
									...
									else if (element3.getTagName().equals("content"))
									{
										...
										// clone the content node without the
										// children of <properties>
										Element el3clone = (Element) element3
												.cloneNode(false);

										// update the assignmentcontent id in
										// assignment node
										String assignContentId = "/assignment/c/" + siteId
												+ "/" + newContentId;
										el2clone.setAttribute("assignmentcontent",
												assignContentId);

										// for content node, process the
										// attachment or properties kids
										...
										for (int i5 = 0; i5 < length5; i5++)
										{
											...
											if (child5.getNodeType() == Node.ELEMENT_NODE)
											{
												...
												if (element5.getTagName()
														.equals("properties"))
												{
													// for the file from WT,
													// preform userId
													// translation when needed
													if (!userIdTrans.isEmpty())
													{
														WTUserIdTrans(element3,
																userIdTrans);
													}
												} // for the node of properties
												el3clone.appendChild(
														element5.cloneNode(true));

												// for "attachment" children
												if (element5.getTagName()
														.equals("attachment"))
												{
													// map the attachment area
													// folder name
													...
													// map the attachment area
													// folder name
													String oldUrl = element5
															.getAttribute("relative-url");
													if (oldUrl.startsWith(
															"/content/attachment/"
																	+ fromSiteId + "/"))
													{
														String newUrl = "/content/attachment/"
																+ siteId
																+ oldUrl.substring(
																		("/content/attachment"
																				+ fromSiteId)
														...
														// attachment if
														// necessary
														newUrl = transferAttachment(
																fromSiteId, siteId, null,
																oldUrl.substring(
																		"/content"
																				.length()));
														element5.setAttribute(
													...
													// map any references to
													// this site to the new site
													// id
													else if (oldUrl
															.startsWith("/content/group/"
																	+ fromSiteId + "/"))
													{
													...
													// to the attribute field of
													// content
													// to satisfy the input need
													// of mergeAssignmentContent
													String attachmentString = "attachment"
															+ attCount;
													el3clone.setAttribute(
															attachmentString,
															element5.getAttribute(
																	"relative-url"));
													attCount++;
							...
							// when importing, refer to property to determine
							// draft status
		...
		{
			M_log.warn(" merge(): exception: " + any.getMessage() + " siteId=" + siteId
					+ " from site id=" + fromSiteId);
		}

		results.append(
				"merging assignment " + siteId + " (" + count + ") assignments.\n");
		return results.toString();
	...
	public Map<String, String> transferCopyEntitiesRefMigrator(String fromContext,
			String toContext, List resourceIds)
	{
		...
		while (oAssignments.hasNext())
		{
			...
			{
				// if there is a list for import assignments, only import those
				// assignments and relative submissions
				toBeImported = false;
			...
			if (toBeImported)
			{
				...
				else
				{
					try
					{
						...
						{
							AssignmentContent oContent = getAssignmentContent(
									oContentReference);
							...
							// replace all occurrence of old context with new
							// context inside instruction text
							String instructions = oContent.getInstructions();
							if (instructions.indexOf(fromContext) != -1)
							{
								instructions = instructions.replaceAll(fromContext,
										toContext);
							}
							...
							// review service
							nContent.setAllowReviewService(
									oContent.getAllowReviewService());
							// properties
							...
							{
								Reference oAttachmentRef = (Reference) oAttachments
										.get(n);
								String oAttachmentId = ((Reference) oAttachments.get(n))
										.getId();
								if (oAttachmentId.indexOf(fromContext) != -1)
								{
									// transfer attachment, replace the context
									// string and add new attachment if
									// necessary
									transferAttachment(fromContext, toContext,
											nAttachments, oAttachmentId);
								}
								else
								{
					...
					{
						if (M_log.isWarnEnabled()) M_log.warn(" transferCopyEntities "
								+ e.toString() + " oAssignmentId=" + oAssignmentId);
					}
					...
					if (nContent != null)
					{
						try
						{
							...
							nAssignment
									.setPosition_order(oAssignment.getPosition_order());

							nAssignment.setAllowPeerAssessment(
									nAssignment.getAllowPeerAssessment());
							nAssignment.setPeerAssessmentAnonEval(
									oAssignment.getPeerAssessmentAnonEval());
							nAssignment.setPeerAssessmentInstructions(
									oAssignment.getPeerAssessmentInstructions());
							nAssignment.setPeerAssessmentNumReviews(
									oAssignment.getPeerAssessmentNumReviews());
							nAssignment.setPeerAssessmentStudentViewReviews(
									oAssignment.getPeerAssessmentStudentViewReviews());
							nAssignment.setPeerAssessmentPeriod(
									oAssignment.getPeerAssessmentPeriod());
							if (nAssignment.getPeerAssessmentPeriod() == null
									&& nAssignment.getCloseTime() != null)
							{
								// set the peer period time to be 10 mins after
								// accept until date
								GregorianCalendar c = new GregorianCalendar();
								c.setTimeInMillis(nAssignment.getCloseTime().getTime());
								c.add(GregorianCalendar.MINUTE, 10);
								nAssignment.setPeerAssessmentPeriod(
										TimeService.newTime(c.getTimeInMillis()));
							}
							// properties
							...
							String associatedGradebookAssignment = StringUtils
									.trimToNull(p.getProperty(
											PROP_ASSIGNMENT_ASSOCIATE_GRADEBOOK_ASSIGNMENT));
							if (associatedGradebookAssignment != null)
							{
								// see if the old assignment's associated
								// gradebook item is an internal gradebook entry
								// or externally defined
								boolean isExternalAssignmentDefined = m_gradebookExternalAssessmentService
										.isExternalAssignmentDefined(
												oAssignment.getContent().getContext(),
												associatedGradebookAssignment);
								if (isExternalAssignmentDefined)
								{
									// if this is an external defined (came from
									// assignment)
									// mark the link as "add to gradebook" for
									// the new imported assignment, since the
									// assignment is still of draft state
									// later when user posts the assignment, the
									// corresponding assignment will be created
									// in gradebook.
									p.removeProperty(
											PROP_ASSIGNMENT_ASSOCIATE_GRADEBOOK_ASSIGNMENT);
									p.addProperty(NEW_ASSIGNMENT_ADD_TO_GRADEBOOK,
											GRADEBOOK_INTEGRATION_ADD);
								}
							}

							// remove the link btw assignment and announcement
							// item. One can announce the open date afterwards
							p.removeProperty(
							...
							// remove the link btw assignment and calendar item.
							// One can add the due date to calendar afterwards
							p.removeProperty(
							...
							M_log.info("old assignment id:" + oAssignment.getId()
									+ " - new assignment id:" + nAssignment.getId());
							...
							{
								M_log.error(this + " transferCopyEntities "
										+ pe.toString() + " oAssignmentId="
										+ oAssignment.getId() + " nAssignmentId="
										+ nAssignment.getId());
							}
						}
						catch (Exception ee)
						{
							M_log.error(this + " transferCopyEntities " + ee.toString()
									+ " oAssignmentId=" + oAssignment.getId()
			...
			List nAttachments, String oAttachmentId)
	{
		...
		try
		{
			ContentResource attachment = m_contentHostingService
		...
		catch (IdUnusedException e)
		{
			try
			{
				ContentResource oAttachment = m_contentHostingService
				...
				catch (Exception eeAny)
				{
					...
					M_log.warn(" transferCopyEntities: cannot add new attachment with id="
							+ nAttachmentId + " " + eeAny.getMessage());
				}
			}
			catch (Exception eAny)
			{
				...
				M_log.warn(
						" transferCopyEntities: cannot find the original attachment with id="
								+ oAttachmentId + " " + eAny.getMessage());
			}
		}
		catch (Exception any)
		{
			M_log.warn(" transferCopyEntities" + any.getMessage() + " oAttachmentId="
					+ oAttachmentId + " nAttachmentId=" + nAttachmentId);
		}
	...
	public String getEntityDescription(Reference ref)
	{
		...
		try
		{
			...
			else if (REF_TYPE_SUBMISSION.equals(ref.getSubType()))
			{
				...
				M_log.warn(" getEntityDescription(): unknown message ref subtype: "
						+ ref.getSubType() + " in ref: " + ref.getReference());
		}
		catch (PermissionException e)
		{
			M_log.warn(" getEntityDescription(): " + e.getMessage() + " ref="
					+ ref.getReference());
		}
		catch (IdUnusedException e)
		{
	...
	public ResourceProperties getEntityResourceProperties(Reference ref)
	{
		...
		try
		{
			...
			else if (REF_TYPE_SUBMISSION.equals(ref.getSubType()))
			{
				...
				M_log.warn(" getEntityResourceProperties: unknown message ref subtype: "
						+ ref.getSubType() + " in ref: " + ref.getReference());
		}
		catch (PermissionException e)
		{
			M_log.warn(" getEntityResourceProperties(): " + e.getMessage() + " ref="
					+ ref.getReference());
		}
		catch (IdUnusedException e)
		{
	...
	public boolean canSubmit(String context, Assignment a, String userId)
	{
		...
		if (!allowAddSubmissionCheckGroups(context, a)
				&& !allowAddAssignment(context) /*
												 * SAK-25555 return true if user
												 * is allowed to add assignment
												 */)
			return false;
		...
		try
		{
			...
			// return false if the current time has passed the assignment close
			// time
			...
			String allowResubmitNumString = submission != null
					? submission.getProperties().getProperty(
							AssignmentSubmission.ALLOW_RESUBMIT_NUMBER)
					: null;
			if (allowResubmitNumString != null && submission.getTimeSubmitted() != null)
			{
				try
				{
					...
					String allowResubmitCloseTime = submission != null
							? (String) submission.getProperties().getProperty(
									AssignmentSubmission.ALLOW_RESUBMIT_CLOSETIME)
							: null;
					...
					{
						// see if a resubmission close time is set on submission
						// level
						resubmitCloseTime = TimeService
								.newTime(Long.parseLong(allowResubmitCloseTime));
					}
					else
					{
						// otherwise, use assignment close time as the
						// resubmission close time
						resubmitCloseTime = a.getCloseTime();
					}
					return (allowResubmitNumber > 0
							/* additional submission number allowed */ || allowResubmitNumber == -1 /*
																									 * unlimited
																									 * submission
																									 * times
																									 * allowed
																									 */)
							&& resubmitCloseTime != null
							&& currentTime.before(resubmitCloseTime);
				}
				catch (NumberFormatException e)
				{
					M_log.warn(" canSubmit(String, Assignment) " + e.getMessage()
							+ " allowResubmitNumString=" + allowResubmitNumString);
				}
			}

			if (submission == null
					|| (submission != null && submission.getTimeSubmitted() == null))
			{
			...
			else
			{
				if (!submission.getSubmitted()
						&& !(closeTime != null && currentTime.after(closeTime)))
				{
	...
	public class BaseAssignment implements Assignment
	{
		...
		public BaseAssignment(Element el)
		{
			...
			M_log.debug(
					" BASE ASSIGNMENT : STORAGE CONSTRUCTOR : ASSIGNMENT ID : " + m_id);
			...
			m_position_order = 0; // prevents null pointer if there is no
									// position_order defined as well as helps
									// with the sorting
			try
			{
				m_position_order = Long.valueOf(el.getAttribute("position_order"))
						.intValue();
			}
			catch (Exception e)
			{
			...
			m_peerAssessmentPeriodTime = getTimeObject(
					el.getAttribute("peerassessmentperiodtime"));
			m_peerAssessmentAnonEval = getBool(el.getAttribute("peerassessmentanoneval"));
			m_peerAssessmentStudentViewReviews = getBool(
					el.getAttribute("peerassessmentstudentviewreviews"));
			...
			M_log.debug(" BASE ASSIGNMENT : STORAGE CONSTRUCTOR : number of authors : "
					+ intString);
			try
			{
				...
				for (int x = 0; x < numAttributes; x++)
				{
					...
					M_log.debug(
							" BASE ASSIGNMENT : STORAGE CONSTRUCTOR : reading author # "
									+ x);
					...
					if (tempString != null)
					{
						...
						M_log.debug(
								" BASE ASSIGNMENT : STORAGE CONSTRUCTOR : adding author # "
										+ x + " id :  " + tempString);
			...
			{
				M_log.warn(
						" BASE ASSIGNMENT : STORAGE CONSTRUCTOR : Exception reading authors : "
								+ e);
			}
			...
			// extract access
			AssignmentAccess access = AssignmentAccess
					.fromString(el.getAttribute("access"));
			if (access != null)
			{
				m_access = access;
			}
		...
		public ContentHandler getContentHandler(Map<String, Object> services)
		{
			...
			return new DefaultEntityHandler()
			{
				 ...
				 * (non-Javadoc)
				 * @see
				 * org.sakaiproject.util.DefaultEntityHandler#startElement(java.
						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if ("assignment".equals(qName) && entity == null)
						{
							...
							M_log.debug(
									this + " getContentHandler: READ THROUGH REG ATTS");

							m_assignmentContent = attributes
									.getValue("assignmentcontent");

							M_log.debug(
									this + " getContentHandler: STORAGE CONSTRUCTOR : CONTENT ID : "
											+ m_assignmentContent);

							m_openTime = getTimeObject(attributes.getValue("opendate"));
							m_dueTime = getTimeObject(attributes.getValue("duedate"));
							m_visibleTime = getTimeObject(
									attributes.getValue("visibledate"));
							m_dropDeadTime = getTimeObject(
									attributes.getValue("dropdeaddate"));
							m_closeTime = getTimeObject(attributes.getValue("closedate"));
							m_context = attributes.getValue("context");
							try
							{
								m_position_order = NumberUtils
										.toInt(attributes.getValue("position_order"));
							}
							catch (Exception e)
							{
								m_position_order = 0; // prevents null pointer
														// if there is no
														// position_order
														// defined as well as
														// helps with the
							...
							m_allowPeerAssessment = getBool(
									attributes.getValue("allowpeerassessment"));
							m_peerAssessmentPeriodTime = getTimeObject(
									attributes.getValue("peerassessmentperiodtime"));
							m_peerAssessmentAnonEval = getBool(
									attributes.getValue("peerassessmentanoneval"));
							m_peerAssessmentStudentViewReviews = getBool(attributes
									.getValue("peerassessmentstudentviewreviews"));
							String numReviews = attributes
									.getValue("peerassessmentnumreviews");
							m_peerAssessmentNumReviews = 0;
							if (numReviews != null && !"".equals(numReviews))
							{
								try
								{
									m_peerAssessmentNumReviews = Integer
											.parseInt(numReviews);
								}
								catch (Exception e)
								{
							...
							m_peerAssessmentInstructions = attributes
									.getValue("peerassessmentinstructions");
							...
							{
								M_log.warn(
										" BASE ASSIGNMENT getContentHandler startElement : Exception reading authors : "
												+ e.toString());
							}

							// extract access
							AssignmentAccess access = AssignmentAccess
									.fromString(attributes.getValue("access"));
							if (access != null)
							{
								m_access = access;
							}
						...
						{
							M_log.debug(
									this + " BaseAssignment getContentHandler Unexpected Element "
											+ qName);
						}
		...
		/**
		 * Takes the Assignment's attribute values and puts them into the xml
		 * document.
		 * 
		 * @param s
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			// SAK-13408 -The XML implementation in Websphere throws an
			// LSException if the
			// attribute is null, while in Tomcat it assumes an empty string.
			// The following
			...
			assignment.setAttribute("assignmentcontent",
					m_assignmentContent == null ? "" : m_assignmentContent);
			...
			assignment.setAttribute("position_order",
					Long.valueOf(m_position_order).toString().trim());
			assignment.setAttribute("allowpeerassessment",
					getBoolString(m_allowPeerAssessment));
			assignment.setAttribute("peerassessmentperiodtime",
					getTimeString(m_peerAssessmentPeriodTime));
			assignment.setAttribute("peerassessmentanoneval",
					getBoolString(m_peerAssessmentAnonEval));
			assignment.setAttribute("peerassessmentstudentviewreviews",
					getBoolString(m_peerAssessmentStudentViewReviews));
			assignment.setAttribute("peerassessmentnumreviews",
					"" + m_peerAssessmentNumReviews);
			assignment.setAttribute("peerassessmentinstructions",
					m_peerAssessmentInstructions);
			...
			M_log.debug(this + " BASE ASSIGNMENT : TOXML : saving " + numItemsString
					+ " authors");
			...
			for (int x = 0; x < m_authors.size(); x++)
			{
				...
				if (itemString != null)
				{
					...
					M_log.debug(this + " BASE ASSIGNMENT : TOXML : saving author : "
							+ itemString);
				}
			}
		...
		protected void setAll(Assignment assignment)
		{
			if (assignment != null)
			{
				...
				{
					M_log.warn(
							": setAll(Assignment) get position order " + e.getMessage());
				}
				m_properties = new BaseResourcePropertiesEdit();
				...
				m_peerAssessmentStudentViewReviews = assignment
						.getPeerAssessmentStudentViewReviews();
		...
		{
			return getAccessPoint(false) + Entity.SEPARATOR + "a" + Entity.SEPARATOR
					+ m_context + Entity.SEPARATOR + m_id;

		} // getUrl

		/**
		 * Access the internal reference which can be used to access the
		 * resource from within the system.
		 * 
		 * @return The the internal reference which can be used to access the
		 *         resource from within the system.
		...
		/**
		 * peer assessment is set for this assignment and the current time falls
		 * between the assignment close time and the peer asseessment period
		 * time
		 * 
		 * @return
		 */
		...
		/**
		 * peer assessment is set for this assignment but the close time hasn't
		 * passed
		 * 
		 * @return
		 */
		...
		/**
		 * peer assessment is set for this assignment but the current time is
		 * passed the peer assessment period
		 * 
		 * @return
		 */
		...
		public String getStatus()
		{
			...
			else if ((this.getCloseTime() != null)
					&& (this.getCloseTime().before(currentTime)))
		...
		public Time getTimeCreated()
		{
			...
			{
				M_log.warn(
						":getTimeCreated() no time property defined " + e.getMessage());
			}
			catch (EntityPropertyTypeException e)
			{
				M_log.warn(
		...
		public Time getTimeLastModified()
		{
			...
			{
				M_log.warn(":getTimeLastModified() no time property defined "
						+ e.getMessage());
			}
			catch (EntityPropertyTypeException e)
			{
		...
		/**
		 * Access the first time at which the assignment can be viewed; may be
		 * null.
		 * 
		 * @return The Time at which the assignment is due, or null if
		 *         unspecified.
		 */
		 ...
		 *
		 * @return The Time at which the Assignment is visible, or null if
		 *         unspecified.
		 */
		...
		/**
		 * Access the drop dead time after which responses to this assignment
		 * are considered late; may be null.
		 * 
		 * @return The Time object representing the drop dead time, or null if
		 *         unspecified.
		 */
		...
		/**
		 * Access the close time after which this assignment can no longer be
		 * viewed, and after which submissions will not be accepted. May be
		 * null.
		 * 
		 * @return The Time after which the Assignment is closed, or null if
		 *         unspecified.
		 */
		...
		/**
		 * Are these objects equal? If they are both Assignment objects, and
		 * they have matching id's, they are.
		 * 
		...
		/**
		 * Make a hash code that reflects the equals() logic as well. We want
		 * two objects, even if different instances, if they have the same id to
		 * hash the same.
		 */
		public int hashCode()
		{
	...
	/**
	 * <p>
	 * BaseAssignmentEdit is an implementation of the CHEF AssignmentEdit
	 * object.
	 * </p>
	 * 
	 * @author University of Michigan, CHEF Software Development Team
	 */
	public class BaseAssignmentEdit extends BaseAssignment
			implements AssignmentEdit, SessionBindingListener
	{
		...
		/**
		 * Set the first time at which the assignment can be viewed; may be
		 * null.
		 * 
		 * @param opentime
		...
		/**
		 * Set the drop dead time after which responses to this assignment are
		...
		/**
		 * Set the time after which this assignment can no longer be viewed, and
		 * after which submissions will not be accepted. May be null.
		 * 
		 * @param closetime
		 *        - The Time after which the Assignment is closed, or null if
		 *        unspecified.
		 */
		...
		public void setGroupAccess(Collection groups) throws PermissionException
		{
			...
			// is there any change? If we are already grouped, and the group
			// list is the same, ignore the call
			if ((m_access == AssignmentAccess.GROUPED)
			...
			{
				M_log.warn(
						" setGroupAccess() called with null context: " + getReference());
				throw new PermissionException(SessionManager.getCurrentSessionUserId(),
						"access:site", getReference());
			}
			...
			EntityCollections.computeAddedRemovedEntityRefsFromNewEntitiesOldRefs(
					addedGroups, removedGroups, groups, m_groups);
			...
			if (removedGroups.size() > 0)
			{
				...
				for (Iterator i = removedGroups.iterator(); i.hasNext();)
				{
					...
					if (!EntityCollections
							.entityCollectionContainsRefString(allowedGroups, ref))
					{
		...
		public void clearGroupAccess() throws PermissionException
		{
			...
			{
				// there should not be a case where there's no context
				M_log.warn(" clearGroupAccess() called with null context. "
						+ getReference());
				throw new PermissionException(SessionManager.getCurrentSessionUserId(),
						"access:site", getReference());
			}
			...
			{
				// verify that the user has permission to add in the site
				// context
				if (!allowAddSiteAssignment(m_context))
				{
					throw new PermissionException(
							SessionManager.getCurrentSessionUserId(), "access:site",
							getReference());
				}
			}
	...
	public class BaseAssignmentContent implements AssignmentContent
	{
		...
		boolean m_checkCompilatio = true;
		...
		public BaseAssignmentContent(Element el)
		{
			...
			m_checkCompilatio = getBool(el.getAttribute("checkCompilatio"));
			...
			m_instructions = FormattedText.decodeFormattedTextAttribute(el,
					"instructions");
			...
			{
				M_log.warn(
						" BaseAssignmentContent Exception parsing honor pledge int from xml file string : "
								+ e);
			}
			...
			{
				M_log.warn(
						" BaseAssignmentContent Exception parsing submission type int from xml file string : "
								+ e);
			}
			...
			{
				M_log.warn(
						" BaseAssignmentContent Exception parsing grade type int from xml file string : "
								+ e);
			}
			...
			{
				// %%%zqian
				// read the scaled max grade point first; if there is none, get
				// the old max grade value and multiple by 10
				String maxGradePoint = StringUtils
						.trimToNull(el.getAttribute("scaled_maxgradepoint"));
				if (maxGradePoint == null)
				{
					maxGradePoint = StringUtils
							.trimToNull(el.getAttribute("maxgradepoint"));
					if (maxGradePoint != null)
					{
			...
			{
				M_log.warn(
						" BaseAssignmentContent Exception parsing maxgradepoint int from xml file string : "
								+ e);
			}
			...
			try
			{
				...
				for (int x = 0; x < numAttributes; x++)
				{
					...
					if (tempString != null)
					{
						...
						M_log.debug(" BaseAssignmentContent: " + attributeString + " : "
								+ tempString);
					}
				}
			}
			...
			{
				M_log.warn(
						" BaseAssignmentContent: Exception reading attachments : " + e);
			}
			...
			for (int i = 0; i < length; i++)
			{
				...
				else if (element.getTagName().equals("instructions-html")
						|| element.getTagName().equals("instructions-formatted")
						|| element.getTagName().equals("instructions"))
				{
							...
							&& (element.getChildNodes().item(0) != null))
					{
						...
						if (element.getTagName().equals("instructions"))
							m_instructions = FormattedText
									.convertPlaintextToFormattedText(m_instructions);
						if (element.getTagName().equals("instructions-formatted"))
							m_instructions = FormattedText
									.convertOldFormattedText(m_instructions);

						M_log.debug(" BaseAssignmentContent(Element): instructions : "
								+ m_instructions);
					}
						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if ("content".equals(qName) && entity == null)
						{
							...
							m_individuallyGraded = getBool(
									attributes.getValue("indivgraded"));
							m_releaseGrades = getBool(
									attributes.getValue("releasegrades"));
							m_allowAttachments = getBool(
									attributes.getValue("allowattach"));
							m_hideDueDate = getBool(attributes.getValue("hideduedate"));
							m_allowReviewService = getBool(
									attributes.getValue("allowreview"));
							m_allowStudentViewReport = getBool(
									attributes.getValue("allowstudentview"));
							m_submitReviewRepo = attributes.getValue("submitReviewRepo");
							m_generateOriginalityReport = attributes
									.getValue("generateOriginalityReport");
							m_checkCompilatio = getBool(
									attributes.getValue("checkCompilatio"));
							String excludeTypeStr = attributes.getValue("excludeType");
							try
							{
								m_excludeType = Integer.parseInt(excludeTypeStr);
								if (m_excludeType != 0 && m_excludeType != 1
										&& m_excludeType != 2)
								{
									m_excludeType = 0;
								}
							...
							m_timeCreated = getTimeObject(
									attributes.getValue("datecreated"));
							m_timeLastModified = getTimeObject(
									attributes.getValue("lastmod"));

							m_instructions = formattedTextDecodeFormattedTextAttribute(
									attributes, "instructions");

							try
							{
								m_honorPledge = Integer
										.parseInt(attributes.getValue("honorpledge"));
							}
							catch (Exception e)
							{
								M_log.warn(
										" getContentHandler startElement Exception parsing honor pledge int from xml file string : "
												+ e);
							}

							try
							{
								m_typeOfSubmission = Integer
										.parseInt(attributes.getValue("submissiontype"));
							}
							catch (Exception e)
							{
								M_log.warn(
										" getContentHandler startElement Exception parsing submission type int from xml file string : "
												+ e);
							}

							try
							{
								m_typeOfGrade = Integer
										.parseInt(attributes.getValue("typeofgrade"));
							}
							catch (Exception e)
							{
								M_log.warn(
										" getContentHandler startElement Exception parsing grade type int from xml file string : "
												+ e);
							}
							...
							{
								// %%%zqian
								// read the scaled max grade point first; if
								// there is none, get the old max grade value
								// and multiple by 10
								String maxGradePoint = StringUtils.trimToNull(
										attributes.getValue("scaled_maxgradepoint"));
								if (maxGradePoint == null)
								{
									maxGradePoint = StringUtils.trimToNull(
											attributes.getValue("maxgradepoint"));
									if (maxGradePoint != null)
									{
										maxGradePoint = maxGradePoint + "0";
									}
								}
								m_maxGradePoint = maxGradePoint != null
										? Integer.parseInt(maxGradePoint)
										: m_maxGradePoint;
							}
							catch (Exception e)
							{
								M_log.warn(
										" getContentHandler startElement Exception parsing maxgradepoint int from xml file string : "
												+ e);
							}
							...
							{
								M_log.warn(
										" getContentHandler startElement Exception reading authors : "
												+ e);
							}
							...
							try
							{
								...
								for (int x = 0; x < numAttributes; x++)
								{
									...
									if (tempString != null)
									{
										tempReference = m_entityManager
												.newReference(tempString);
										m_attachments.add(tempReference);
									}
								}
							}
							...
							{
								M_log.warn(
										" getContentHandler startElement DbCachedContent : Exception reading attachments : "
												+ e);
							}
						...
						{
							M_log.warn(
									" getContentHandler startElement Unexpected Element "
											+ qName);
						}
					}
				}
		...
		/**
		 * Takes the AssignmentContent's attribute values and puts them into the
		 * xml document.
		 * 
		 * @param s
		 *        - Data structure holding the object to be stored.
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			content.setAttribute("allowstudentview",
					getBoolString(m_allowStudentViewReport));
			content.setAttribute("submitReviewRepo", m_submitReviewRepo);
			content.setAttribute("generateOriginalityReport",
					m_generateOriginalityReport);
			content.setAttribute("checkCompilatio", getBoolString(m_checkCompilatio));
			...
			FormattedText.encodeFormattedTextAttribute(content, "instructions",
					m_instructions);
		...
		protected void setAll(AssignmentContent content)
		{
			if (content != null)
			{
				...
				m_checkCompilatio = content.isCheckCompilatio();
		...
		public String getUrl()
		{
			return getAccessPoint(false) + Entity.SEPARATOR + "c" + Entity.SEPARATOR
					+ m_context + Entity.SEPARATOR + m_id;
		 ...
		 * 
		 * @return The set of attachments (a ReferenceVector containing
		 *         Reference objects) (may be empty).
		 */
		...
		/**
		 * Get the maximum grade for grade type = SCORE_GRADE_TYPE(3) Formated
		...
		public String getMaxGradePointDisplay()
		{
			...
			String one_decimal_maxGradePoint = m_maxGradePoint / 10 + "."
					+ (m_maxGradePoint % 10);
		 ...
		 * 
		 * @return individGraded - true if projects are individually graded,
		 *         false if grades are given to the group.
		 */
		 ...
		 * 
		 * @return true if grades can be released once submission are graded,
		 *         false if they must be released manually.
		 */
		...
		/**
		 * Get the Honor Pledge type; values are NONE and
		 * ENGINEERING_HONOR_PLEDGE.
		 * 
		 ...
		 * 
		 * @return true if the Assignment allows review service, false
		 *         otherwise?
		 */
		...
		public boolean isCheckCompilatio()
		{
			return m_checkCompilatio;
		}

		public void setCheckCompilatio(boolean m_checkCompilatio)
		{
			this.m_checkCompilatio = m_checkCompilatio;
		}
	...
	/**
	 * <p>
	 * BaseAssignmentContentEdit is an implementation of the CHEF
	 * AssignmentContentEdit object.
	 * </p>
	 * 
	 * @author University of Michigan, CHEF Software Development Team
	 */
	public class BaseAssignmentContentEdit extends BaseAssignmentContent
			implements AttachmentContainer, AssignmentContentEdit, SessionBindingListener
	{
		 ...
		 * 
		 * @param ref
		 *        - The attachment Reference to remove (the one removed will
		 *        equal this, they need not be ==).
		 */
		 ...
		 * 
		 * @param attachments
		 *        - A ReferenceVector that will become the new set of
		 *        attachments.
		 */
		 ...
		 * 
		 * @param individGraded
		 *        - true if projects are individually graded, false if grades
		 ...
		 * 
		 * @param release
		 *        - true if grades can be released once submission are graded,
		 *        false if they must be released manually.
		 */
		...
		/**
		 * Set the Honor Pledge type; values are NONE and
		 * ENGINEERING_HONOR_PLEDGE.
		 * 
		 * @param pledgeType
		 ...
		 * 
		 * @param allow
		 *        - true if the Assignment allows review service, false
		 *        otherwise?
		 */
		 ...
		 * 
		 * @param allow
		 *        - true if the Assignment allows students to view the report,
		 *        false otherwise?
		 */
	...
	public class BaseAssignmentSubmission implements AssignmentSubmission
	{
		...
		// The externalId given by the review service
		...
		public int getReviewScore()
		{
			...
			M_log.debug(this + " getReviewScore for submission " + this.getId()
					+ " and review service is: "
					+ (this.getAssignment().getContent().getAllowReviewService()));
			if (!this.getAssignment().getContent().getAllowReviewService())
			{
				M_log.debug(this
						+ " getReviewScore Content review is not enabled for this assignment");
				return -2;
			}
			...
			else
			{
				...
				{
					M_log.debug(this
							+ " getReviewScore No suitable attachments found in list");
					return -2;
				}
				...
				try
				{
					...
					M_log.debug(this + " getReviewScore checking for score for content: "
							+ contentId);

					Long status = contentReviewService.getReviewStatus(contentId);
					if (status != null
							&& (status.equals(ContentReviewItem.NOT_SENDED_CODE)))
					{
						M_log.debug(this + " getReviewStatus returned a status of: "
								+ status);
						return -4;
					}
					if (status != null
							&& (status.equals(ContentReviewItem.NOT_SUBMITTED_CODE)))
					{
						M_log.debug(this + " getReviewStatus returned a status of: "
								+ status);
						return -2;
					...
					if (status != null && (status
							.equals(ContentReviewItem.SUBMITTED_AWAITING_REPORT_CODE)))
					{
						M_log.debug(this + " getReviewStatus returned a status of: "
								+ status);
						return -3;
					...
					m_reviewScore = score;
					M_log.debug(
							this + " getReviewScore CR returned a score of: " + score);
					return score;
					...
					// try {
					//
					// M_log.debug(this + " getReviewScore Item is not in queue
					// we will try add it");
					// String contentId = cr.getId();
					// String userId = this.getSubmitterId();
					// try {
					// contentReviewService.queueContent(userId, null,
					// getAssignment().getReference(), contentId);
					// } catch (QueueException qe) {
					// M_log.warn(" getReviewScore Unable to queue content with
		...
		{
			// Code to get updated score if default
			M_log.debug(this + " getContentReviewExternalId for submission "
					+ this.getId() + " and review service is: "
					+ (this.getAssignment().getContent().getAllowReviewService()));
			if (!this.getAssignment().getContent().getAllowReviewService())
			{
				M_log.debug(this
						+ " getContentReviewExternalId Content review is not enabled for this assignment");
				return null;
			}
			...
			{
				// we may have already retrived this one
				if (m_externalId != null && m_externalId.length() > 0)
				{
					M_log.debug("returning stored value of " + m_externalId);
					return m_externalId;
				}
				...
				{
					M_log.debug(this
							+ " getContentReviewExternalId No suitable attachments found in list");
					return null;
				}
				...
				try
				{
					...
					String contentId = cr.getId();
					M_log.debug(
							this + " getContentReviewExternalId checking for score for content: "
									+ contentId);

					String externalId = contentReviewService.getExternalId(contentId);
					m_externalId = externalId;
					M_log.debug(
							this + " getContentReviewExternalId CR returned a externalId of: "
									+ externalId);
					return externalId;
		...
		public String startAnalyse()
		{
			...
			else
			{
				try
				{
					...
					{
						M_log.debug(this
								+ " getReviewReport No suitable attachments found in list");
						return "error";
					}

					String contentId = cr.getId();
					return contentReviewService.startAnalyse(contentId);
				}
				catch (Exception e)
				{
		...
		public String getReviewError()
		{
			...
			else
			{
				try
				{
					...
					{
						M_log.debug(this
								+ " getReviewError No suitable attachments found in list");
						return null;
					}
					...
					if (status != null)
					{
						...
						else if (status.equals(ContentReviewItem.REPORT_ERROR_RETRY_CODE))
						{
							errorMessage = rb.getString(
								...
								.equals(ContentReviewItem.SUBMISSION_ERROR_NO_RETRY_CODE))
						{
							errorMessage = rb.getString(
									"content_review.error.SUBMISSION_ERROR_NO_RETRY_CODE");
						}
						else if (status
								.equals(ContentReviewItem.SUBMISSION_ERROR_RETRY_CODE))
						{
							errorMessage = rb.getString(
									"content_review.error.SUBMISSION_ERROR_RETRY_CODE");
						}
						else if (status.equals(
								ContentReviewItem.SUBMISSION_ERROR_RETRY_EXCEEDED))
						{
							errorMessage = rb.getString(
									"content_review.error.SUBMISSION_ERROR_RETRY_EXCEEDED_CODE");
						}
						else if (status.equals(
								ContentReviewItem.SUBMISSION_ERROR_USER_DETAILS_CODE))
						{
							errorMessage = rb.getString(
									"content_review.error.SUBMISSION_ERROR_USER_DETAILS_CODE");
						}
						else if (ContentReviewItem.SUBMITTED_AWAITING_REPORT_CODE
								.equals(status)
								|| ContentReviewItem.NOT_SUBMITTED_CODE.equals(status))
						{
							// recupertion de la progression
							int progression = contentReviewService
									.getReviewScore(contentId);
							if (progression == 0)
							{
								errorMessage = rb
										.getString("content_review.pending.info");
							}
							else
							{
								errorMessage = rb.getFormattedMessage(
										"content_review.pendingProgression.info",
										new String[] { String.valueOf(progression) });
							}
						}
					}
					Integer errorCode = contentReviewService
							.getReviewErrorCode(contentId);
					if (errorCode != null && errorCode > 0)
					{
						if (ContentReviewItem.Error.find(errorCode) != null)
						{
							String errorMsg = "";
							...
							errorMessage = rb
									.getString("compilatio."
											+ ContentReviewItem.Error.find(errorCode))
									+ " " + errorMsg;
						}
					}
				...
				catch (QueueException e)
				{
					...
					return rb.getString("compilatio.queueException");
				}
				catch (Exception e)
				{
					// e.printStackTrace();
		...
		{
			if (m_reviewIconUrl == null) m_reviewIconUrl = contentReviewService
					.getIconUrlforScore(Long.valueOf(this.getReviewScore()));
		 ...
		 */
		public BaseAssignmentSubmission(String id, String assignId, String submitterId,
				String submitTime, String submitted, String graded)
		{
		...
		public BaseAssignmentSubmission(Element el)
		{
			...
			// %%%zqian
			// read the scaled grade point first; if there is none, get the old
			// grade value
			...
			if (grade == null)
			{
				...
				if (grade != null)
				{
					...
					{
						M_log.warn(":BaseAssignmentSubmission(Element el) "
								+ e.getMessage());
					}
				}
			}
			...
			m_submittedText = FormattedText.decodeFormattedTextAttribute(el,
					"submittedtext");
			m_feedbackComment = FormattedText.decodeFormattedTextAttribute(el,
					"feedbackcomment");
			m_feedbackText = FormattedText.decodeFormattedTextAttribute(el,
					"feedbacktext");
			...
			{
				M_log.debug(
						" BaseAssignmentSubmission: CONSTRUCTOR : Exception reading logs : "
								+ e);
			}
			...
			{
				M_log.warn(
						" BaseAssignmentSubmission: CONSTRUCTOR : Exception reading grades : "
								+ e);
			}
			...
			m_submitters = new ArrayList();
			M_log.debug(
					" BaseAssignmentSubmission : CONSTRUCTOR : Reading submitters : ");
			...
			{
				M_log.debug(
						" BaseAssignmentSubmission: CONSTRUCTOR : Exception reading submitters : "
								+ e);
			}
			...
			M_log.debug(
					" BaseAssignmentSubmission: CONSTRUCTOR : num feedback attachments : "
							+ intString);
			try
			{
				numAttributes = Integer.parseInt(intString);
				...
				for (int x = 0; x < numAttributes; x++)
				{
					...
					if (tempString != null)
					{
						...
						M_log.debug(" BaseAssignmentSubmission: CONSTRUCTOR : "
								+ attributeString + " : " + tempString);
					}
				}
			}
			catch (Exception e)
			{
				M_log.warn(
						" BaseAssignmentSubmission: CONSTRUCTOR : Exception reading feedback attachments : "
								+ e);
			}
			...
			M_log.debug(
					" BaseAssignmentSubmission: CONSTRUCTOR : num submitted attachments : "
							+ intString);
			try
			{
				numAttributes = Integer.parseInt(intString);
			...
			{
				M_log.debug(
						" BaseAssignmentSubmission: CONSTRUCTOR : Exception reading submitted attachments : "
								+ e);
			}

			// READ THE PROPERTIES, SUBMITTED TEXT, FEEDBACK COMMENT, FEEDBACK
			// TEXT
			NodeList children = el.getChildNodes();
			final int length = children.getLength();
			for (int i = 0; i < length; i++)
			{
				...
				else if (element.getTagName().equals("submittedtext"))
				{
							...
							&& (element.getChildNodes().item(0) != null))
					{
						...
						M_log.debug(
								" BaseAssignmentSubmission: CONSTRUCTOR : submittedtext : "
										+ m_submittedText);
					}
				...
				else if (element.getTagName().equals("feedbackcomment"))
				{
					if ((element.getChildNodes() != null)
							&& (element.getChildNodes().item(0) != null))
					{
						m_feedbackComment = element.getChildNodes().item(0)
								.getNodeValue();

						M_log.debug(
								" BaseAssignmentSubmission: CONSTRUCTOR : feedbackcomment : "
										+ m_feedbackComment);
					}
				...
				else if (element.getTagName().equals("feedbacktext"))
				{
							...
							&& (element.getChildNodes().item(0) != null))
					{
						...
						M_log.debug(
								" BaseAssignmentSubmission: CONSTRUCTOR : FEEDBACK TEXT : "
										+ m_feedbackText);
					}
			...
			// get the review Status from ContentReview rather than using old
			// ones
			if (contentReviewService != null)
			{
						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if ("submission".equals(qName) && entity == null)
						{
							...
							m_id = attributes.getValue("id");
							// M_log.info(this + " BASE SUBMISSION : CONSTRUCTOR
							// : m_id : " + m_id);
							m_context = attributes.getValue("context");
							// M_log.info(this + " BASE SUBMISSION : CONSTRUCTOR
							// : m_context : " + m_context);
							...
							// %%%zqian
							// read the scaled grade point first; if there is
							// none, get the old grade value
							String grade = StringUtils
									.trimToNull(attributes.getValue("scaled_grade"));
							if (grade == null)
							{
								grade = StringUtils
										.trimToNull(attributes.getValue("grade"));
								if (grade != null)
								{
									try
									{
										Integer.parseInt(grade);
										// for the grades in points, multiple
										// those by 10
										grade = grade + "0";
									}
									catch (Exception e)
									{
										M_log.warn(
												":BaseAssignmentSubmission:getContentHanler:DefaultEnityHandler "
														+ e.getMessage());
									}
								}
							}
							...
							m_timeSubmitted = getTimeObject(
									attributes.getValue("datesubmitted"));
							m_timeReturned = getTimeObject(
									attributes.getValue("datereturned"));
							m_assignment = attributes.getValue("assignment");
							m_timeLastModified = getTimeObject(
									attributes.getValue("lastmod"));
							...
							m_gradeReleased = getBool(
									attributes.getValue("gradereleased"));
							m_honorPledgeFlag = getBool(
									attributes.getValue("pledgeflag"));
							m_hideDueDate = getBool(attributes.getValue("hideduedate"));

							m_submittedText = formattedTextDecodeFormattedTextAttribute(
									attributes, "submittedtext");
							m_feedbackComment = formattedTextDecodeFormattedTextAttribute(
									attributes, "feedbackcomment");
							m_feedbackText = formattedTextDecodeFormattedTextAttribute(
									attributes, "feedbacktext");

							// SAK-17606
							m_anonymousSubmissionId = m_id.substring(27) + " ("
									+ rb.getString("grading.anonymous.title") + ")";

							// SAK-29314
							m_isUserSubmission = getBool(
									attributes.getValue(SUBMISSION_ATTR_IS_USER_SUB));
							...
							{
								M_log.debug(
										" BaseAssignmentSubmission: error parsing 'numberoflogs' attribute : "
												+ e);
							}
							...
							{
								M_log.warn(
										" BaseAssignmentSubmission: error parsing 'numberofgrades' property : "
												+ e);
							}
							...
							try
							{
								...
								for (int x = 0; x < numAttributes; x++)
								{
									...
									// for backward compatibility of assignments
									// without submitter ids
							...
							{
								M_log.warn(
										" BaseAssignmentSubmission getContentHandler : Exception reading submitters : "
												+ e);
							}

							// READ THE FEEDBACK ATTACHMENTS
							m_feedbackAttachments = m_entityManager.newReferenceList();
							intString = attributes
									.getValue("numberoffeedbackattachments");
							try
							{
								numAttributes = NumberUtils.toInt(intString);
							...
							{
								M_log.warn(
										" BaseAssignmentSubmission getContentHandler : Exception reading feedback attachments : "
												+ e);
							}
							...
							m_submittedAttachments = m_entityManager.newReferenceList();
							intString = attributes
									.getValue("numberofsubmittedattachments");
							try
							{
								numAttributes = NumberUtils.toInt(intString);
							...
							{
								M_log.warn(
										" BaseAssignmentSubmission getContentHandler: Exception reading submitted attachments : "
												+ e);
							}
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			submission.setAttribute("assignment",
					m_assignment == null ? "" : m_assignment);
			...
			// SAK-29314
			submission.setAttribute(SUBMISSION_ATTR_IS_USER_SUB,
					getBoolString(m_isUserSubmission));

			if (M_log.isDebugEnabled())
				M_log.debug(this + " BaseAssignmentSubmission: SAVED REGULAR PROPERTIES");

			submission.setAttribute("submitterid",
					m_submitterId == null ? "" : m_submitterId);

			if (M_log.isDebugEnabled()) M_log.debug(this
					+ " BaseAssignmentSubmission: SAVED SUBMITTER ID : " + m_submitterId);
			...
			if (M_log.isDebugEnabled()) M_log.debug(
					this + " BaseAssignmentSubmission: # grades " + numItemsString);
			...
			if (M_log.isDebugEnabled()) M_log.debug(
					this + " BaseAssignmentSubmission: # submitters " + numItemsString);
			...
			if (M_log.isDebugEnabled()) M_log
					.debug("DB : DbCachedStorage : DbCachedAssignmentSubmission : entering fb attach loop : size : "
							+ numItemsString);
			...
			if (M_log.isDebugEnabled()) M_log.debug(
					this + " BaseAssignmentSubmission: SAVED FEEDBACK ATTACHMENTS");
			...
			if (M_log.isDebugEnabled()) M_log.debug(
					this + " BaseAssignmentSubmission: SAVED SUBMITTED ATTACHMENTS");

			// SAVE THE PROPERTIES
			m_properties.toXml(doc, stack);
			stack.pop();

			FormattedText.encodeFormattedTextAttribute(submission, "submittedtext",
					m_submittedText);
			FormattedText.encodeFormattedTextAttribute(submission, "feedbackcomment",
					m_feedbackComment);
			FormattedText.encodeFormattedTextAttribute(submission, "feedbacktext",
					m_feedbackText);
		...
		public String getUrl()
		{
			return getAccessPoint(false) + Entity.SEPARATOR + "s" + Entity.SEPARATOR
					+ m_context + Entity.SEPARATOR + m_id;
		...
		{
			if (m_asn == null && m_assignment != null) // lazy load assignment
														// as needed, store for
														// future
			{
			...
			// track event
			// EventTrackingService.post(EventTrackingService.newEvent(AssignmentConstants.EVENT_ACCESS_ASSIGNMENT,
			// retVal.getReference(), false));

			return m_asn;
		}

		/**
		 * call this method to store the assignment object to avoid costly
		 * lookup by assignment id later will do nothing if assignment ids don't
		 * match
		 */
		public void setAssignment(Assignment value)
		{
			if (m_assignment != null && value != null
					&& m_assignment.equals(value.getId()))
			{
				m_asn = value;
			}
		}
		...
		public String getGradeForUser(String id)
		{
			if (m_grades != null)
			{
				...
				while (_it.hasNext())
				{
					...
					if (_s.startsWith(id + "::"))
					{
						return _s.endsWith("null") ? null
								: _s.substring(_s.indexOf("::") + 2);
					}
				}
			}
			return null;
		...
		public User[] getSubmitters()
		{
			...
			for (String userId : (List<String>) getSubmitterIds())
			{
				...
				{
					M_log.warn(" BaseAssignmentSubmission getSubmitters" + e.getMessage()
							+ userId);
				}
			}
		...
		/**
		 * Access the list of Users who submitted this response to the
		 * Assignment.
		 * 
		 * @return FlexStringArray of user ids.
		 */
		public List getSubmitterIds()
		{
			...
			if (a.isGroup())
			{
				try
				{
					...
					if (_g != null)
					{
						return getSubmitterIdList("false", _g.getId(), null,
								a.getReference(), a.getContext());
					}
				}
		...
		/**
		 * Set the time at which this response was submitted; null signifies the
		 * response is unsubmitted.
		 * 
		 ...
		 * 
		 * @return True if the Submissions's grade has been released, false
		 *         otherwise.
		 */
		...
		public String getGrade(boolean overrideWithGradebookValue)
		{
			...
			else
			{
				...
				String gAssignmentName = StringUtils
						.trimToNull(m.getProperties().getProperty(
								AssignmentService.PROP_ASSIGNMENT_ASSOCIATE_GRADEBOOK_ASSIGNMENT));
				if (gAssignmentName != null)
				{
					...
					SecurityAdvisor securityAdvisor = new MySecurityAdvisor(
							SessionManager.getCurrentSessionUserId(),
							new ArrayList<String>(Arrays.asList("gradebook.gradeAll",
									"gradebook.gradeSection", "gradebook.editAssignments",
									"gradebook.viewOwnGrades")),
							gradebookUid);
					try
					{
						// add the grade permission ("gradebook.gradeAll",
						// "gradebook.gradeSection",
						// "gradebook.editAssignments", or
						// "gradebook.viewOwnGrades") in order to use
						// g.getAssignmentScoreString()
						securityService.pushAdvisor(securityAdvisor);

						if (g.isGradebookDefined(gradebookUid)
								&& g.isAssignmentDefined(gradebookUid, gAssignmentName))
						{
							String gString = StringUtils
									.trimToNull(g.getAssignmentScoreString(gradebookUid,
											gAssignmentName, userId));
							if (gString != null)
							{
								// return grade with locale decimal separator
								String decSeparator = FormattedText.getDecimalSeparator();
								rv = StringUtils.replace(gString,
										(",".equals(decSeparator) ? "." : ","),
										decSeparator);
								NumberFormat nbFormat = FormattedText.getNumberFormat(1,
										1, false);
					...
					{
						M_log.warn(
								" BaseAssignmentSubmission getGrade getAssignmentScoreString from GradebookService "
										+ e.getMessage() + " context=" + m_context
										+ " assignment id=" + m_assignment + " userId="
										+ userId + " gAssignmentName=" + gAssignmentName);
					}
					finally
					{
						// remove advisor
		...
		public String getGradeDisplay(int typeOfGrade)
		{
			...
			if (typeOfGrade == Assignment.SCORE_GRADE_TYPE)
			{
				if (grade != null && grade.length() > 0 && !"0".equals(grade))
				{
					...
					{
						Integer.parseInt(grade);
						// if point grade, display the grade with one decimal
						// place
						one_decimal_gradePoint = grade.substring(0, grade.length() - 1)
								+ "." + grade.substring(grade.length() - 1);
					}
					catch (NumberFormatException e)
					{
						try
		 ...
		 * 
		 * @return ReferenceVector of the list of attachments as Reference
		 *         objects;
		 */
		...
		/**
		 * Access the formatted text part of the instructors feedback; usually
		 * an annotated copy of the submittedText
		 * 
		...
		/**
		 * Access the list of attachments returned to the students in the
		 * process of grading this assignment; usually a modified or annotated
		 * version of the attachment submitted.
		 * 
		 * @return ReferenceVector of the Resource objects pointing to the
		 *         attachments.
		 */
		 ...
		 * 
		 * @return True if this response was rejected by the grader, false
		 *         otherwise.
		 */
		...
		/**
		 * Get the time on which the graded submission was returned; null means
		 * the response is not yet graded.
		 * 
		...
		/**
		 * Returns the status of the submission : Not Started, submitted,
		 * returned or graded.
		 * 
		...
		public String getStatus()
		{
			...
			boolean allowGrade = assignment != null
					? allowGradeSubmission(assignment.getReference()) : false;
			String retVal = "";
			...
			else
			{
				if (getGraded())
				{
					if (getReturned())
					{
						if (lastModTime != null && returnTime != null
								&& lastModTime.after(TimeService
										.newTime(returnTime.getTime() + 1000 * 10))
								&& !allowGrade)
						{
							// working on a returned submission now
							retVal = rb.getString("gen.dra2") + " "
									+ rb.getString("gen.inpro");
						}
						else
						{
							// not submitted submmission has been graded and
							// returned
							retVal = rb.getString("gen.returned");
						}
					}
		...
		/**
		 * Are these objects equal? If they are both AssignmentSubmission
		 * objects, and they have matching id's, they are.
		 * 
		 * @return true if they are equal, false if not.
		 */
		public boolean equals(Object obj)
		...
		public int compareTo(Object obj)
		{
			...
			// start the compare by comparing their sort names
			int compare = getTimeSubmitted().toString().compareTo(
					((AssignmentSubmission) obj).getTimeSubmitted().toString());
		...
		{
			String numString = StringUtils.trimToNull(
					m_properties.getProperty(AssignmentSubmission.ALLOW_RESUBMIT_NUMBER));
		...
		public Time getCloseTime()
		{
			String closeTimeString = StringUtils.trimToNull(m_properties
		...
		/**
		 * SAK-17606 - Method to return a speacialized string for anonymous
		 * grading.
		 * 
		 * @return
		 */
		...
		/**
		 * SAK-29314 - Determines whether this submission was submitted by a
	...
	/**
	 * <p>
	 * BaseAssignmentSubmissionEdit is an implementation of the CHEF
	 * AssignmentSubmissionEdit object.
	 * </p>
	 * 
	 * @author University of Michigan, CHEF Software Development Team
	 */
	public class BaseAssignmentSubmissionEdit extends BaseAssignmentSubmission
			implements AssignmentSubmissionEdit, SessionBindingListener
	{
		 ...
		 */
		public BaseAssignmentSubmissionEdit(String id, String assignmentId,
				String submitterId, String submitTime, String submitted, String graded)
		{
		...
		/**
		 * Set the time at which this response was submitted; setting it to null
		 * signifies the response is unsubmitted.
		 * 
		 * @param timeSubmitted
		 *        - Time of submission.
		 */
		 ...
		 * 
		 * @param released
		 *        - True if the Submissions's grade has been released, false
		 *        otherwise.
		 */
		...
		/**
		 * Set the text part of the instructors feedback; usually an annotated
		 * copy of the submittedText
		 * 
		 * @param feedback
		 ...
		 * 
		 * @param returned
		 *        - true if this response was rejected by the grader, false
		 *        otherwise.
		 */
		...
		/**
		 * set the grader id (used to distinguish between auto and instructor
		 * grading)
		 */
		...
		/**
		 * Set the time at which the graded Submission was returned; setting it
		 * to null means it is not yet graded.
		 * 
		 * @param timeReturned
		...
		public String postAttachment(List attachments)
		{
			...
			try
			{
				...
				if (ass != null && cr != null)
				{
					return contentReviewService.queueContent(null, null,
							ass.getReference(), cr.getId());
				}
				else
				{
					// error, assignment couldn't be found. Log the error
					M_log.debug(this
							+ " BaseAssignmentSubmissionEdit postAttachment: Unable to find assignment associated with submission id= "
							+ this.m_id + " and assignment id=" + this.m_assignment);
				}
			}
			catch (QueueException qe)
			{
				M_log.warn(
						" BaseAssignmentSubmissionEdit postAttachment: Unable to add content to Content Review queue: "
								+ qe.getMessage());
			}
			catch (Exception e)
			{
		...
		private ContentResource getFirstAcceptableAttachement(List attachments)
		{
			...
			for (int i = 0; i < attachments.size(); i++)
			{
				...
				try
				{
					ContentResource res = m_contentHostingService
							.getResource(attachment.getId());
	...
	protected interface AssignmentStorage
	{
		 ...
		 * @param context
		 *        The context.
		 * @return The locked Assignment object with this id, or null if the id
		 *         is in use.
		 */
		public AssignmentEdit put(String id, String context);

		/**
		 * Get a lock on the Assignment with this id, or null if a lock cannot
		 * be gotten.
		 * 
		 * @param id
		 *        The Assignment id.
		 * @return The locked Assignment with this id, or null if this records
		 *         cannot be locked.
		 */
	...
	protected interface AssignmentContentStorage
	{
		 ...
		 * @param context
		 *        The context.
		 * @return The locked AssignmentContent object with this id, or null if
		 *         the id is in use.
		 */
		public AssignmentContentEdit put(String id, String context);

		/**
		 * Get a lock on the AssignmentContent with this id, or null if a lock
		 * cannot be gotten.
		 * 
		 * @param id
		 *        The AssignmentContent id.
		 * @return The locked AssignmentContent with this id, or null if this
		 *         records cannot be locked.
		 */
	...
	protected interface AssignmentSubmissionStorage
	{
		 ...
		 * @param id
		 *        The AssignmentSubmission id.
		 * @return true if a AssignmentSubmission by this id exists, false if
		 *         not.
		 */
		public boolean check(String id);
		...
		/**
		 * Get the number of submissions which has not been submitted and
		 * graded.
		 * 
		 * @param assignment
		 *        - the Assignment who's submissions you would like.
		 ...
		 * @param context
		 *        The context.
		 * @return The locked AssignmentSubmission object with this id, or null
		 *         if the id is in use.
		 */
		public AssignmentSubmissionEdit put(String id, String assignmentId,
				String submitterId, String submitTime, String submitted, String graded);

		/**
		 * Get a lock on the AssignmentSubmission with this id, or null if a
		 * lock cannot be gotten.
		 * 
		 * @param id
		 *        The AssignmentSubmission id.
		 * @return The locked AssignmentSubmission with this id, or null if this
		 *         records cannot be locked.
		 */
	...
	/**
	 * Utility function which returns the string representation of the long
	 ...
	 * 
	 * @param timeString
	 *        - String version of a time in long format, representing the
	 *        standard ms since the epoch, Jan 1, 1970 00:00:00.
	...
	protected Time getTimeObject(String timeString)
	{
		...
		if (timeString != null)
		{
			...
			catch (Exception e)
			{
				...
				{
					M_log.warn(
							" getTimeObject Base Exception creating time object from xml file : "
									+ ee.getMessage() + " timeString=" + timeString);
				}
			}
		}
		return aTime;
	...
	protected class AssignmentStorageUser implements SingleStorageUser, SAXEntityReader
	{
		 ...
		 * 
		 * @param container
		 *        The Resource that is the container for the new resource (may
		 *        be null).
		 * @param id
		...
		/**
		 * Collect the fields that need to be stored outside the XML (for the
		 * resource).
		 * 
		 * @return An array of field values to store in the record outside the
		 *         XML (for the resource).
		 */
		 ...
		 * (non-Javadoc)
		 * @see
		 * org.sakaiproject.util.SAXEntityReader#getDefaultHandler(java.util.
		 * Map)
		 */
						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if (entity == null)
						{
							...
							{
								M_log.warn(
										" AssignmentStorageUser getDefaultHandler startElement Unexpected Element in XML ["
												+ qName + "]");
							}
			...
			implements SingleStorageUser, SAXEntityReader
	{
		...
		public Edit newResourceEdit(Entity container, String id, Object[] others)
		{
			BaseAssignmentContentEdit e = new BaseAssignmentContentEdit(id,
					(String) others[0]);
			e.activate();
			return e;
		}
		...
		public Edit newResourceEdit(Entity container, Entity other)
		{
			BaseAssignmentContentEdit e = new BaseAssignmentContentEdit(
					(AssignmentContent) other);
			e.activate();
			return e;
		}
						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if (entity == null)
						{
							...
							{
								M_log.warn(
										" AssignmentContentStorageUser getDefaultEntityHandler startElement Unexpected Element in XML ["
												+ qName + "]");
							}
			...
			implements SingleStorageUser, SAXEntityReader
	{
		...
		public Entity newResource(Entity container, String id, Object[] others)
		{
			return new BaseAssignmentSubmission(id, (String) others[0],
					(String) others[1], (String) others[2], (String) others[3],
					(String) others[4]);
		}
		...
		public Edit newResourceEdit(Entity container, String id, Object[] others)
		{
			BaseAssignmentSubmissionEdit e = new BaseAssignmentSubmissionEdit(id,
					(String) others[0], (String) others[1], (String) others[2],
					(String) others[3], (String) others[4]);
			e.activate();
			return e;
		}
		...
		public Edit newResourceEdit(Entity container, Entity other)
		{
			BaseAssignmentSubmissionEdit e = new BaseAssignmentSubmissionEdit(
					(AssignmentSubmission) other);
			e.activate();
			return e;
		}
						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if (entity == null)
						{
							...
							{
								M_log.warn(
										" AssignmentSubmissionStorageUser getDefaultHandler startElement: Unexpected Element in XML ["
												+ qName + "]");
							}
		...
		public int compare(Object o1, Object o2)
		{
			...
			else if ("sortname".equals(m_criteria))
			{
				...
				if (userId1 != null)
				{
					...
					{
						M_log.warn(" AssignmentComparator.compare " + e.getMessage()
								+ " id=" + userId1);
					}
				}
		...
		private String getSubmitterSortname(Object o2)
		{
			...
			if (o2 instanceof AssignmentSubmission)
			{
				...
				if (_submission.getAssignment().isGroup())
				{
					...
					try
					{
						Site _site = SiteService
								.getSite(_submission.getAssignment().getContext());
			...
			Map<String, String> transversalMap)
	{
		if (transversalMap != null && transversalMap.size() > 0)
		{
			...
			while (assignmentsIter.hasNext())
			{
				...
				try
				{
					...
					SecurityAdvisor securityAdvisor = new MySecurityAdvisor(
							...
							new ArrayList<String>(
									Arrays.asList(SECURE_UPDATE_ASSIGNMENT_CONTENT)),
					...
					try
					{
						if (!msgBody.equals(msgBodyPreMigrate.toString()))
						{
							...
							AssignmentContentEdit cEdit = editAssignmentContent(
									assignment.getContentReference());
					...
					catch (Exception e)
					{
						// exception
						M_log.warn(
								"UpdateEntityReference: cannot get assignment content for "
										+ assignment.getId() + e.getMessage());
					}
					finally
					{
				...
				{
					M_log.warn(
							"UpdateEntityReference: remove Assignment and all references for "
	...
	public Map<String, String> transferCopyEntitiesRefMigrator(String fromContext,
			String toContext, List ids, boolean cleanup)
	{
		Map<String, String> transversalMap = new HashMap<String, String>();
		...
		try
		{
			if (cleanup == true)
			{
				...
				while (assignmentsIter.hasNext())
				{
					...
					SecurityAdvisor securityAdvisor = new MySecurityAdvisor(
							SessionManager.getCurrentSessionUserId(),
							new ArrayList<String>(Arrays.asList(SECURE_UPDATE_ASSIGNMENT,
					...
					{
						M_log.warn(
								":transferCopyEntities: remove Assignment and all references for "
										+ assignment.getId() + ee.getMessage());
					}
					finally
					{
			...
			transversalMap
					.putAll(transferCopyEntitiesRefMigrator(fromContext, toContext, ids));
		}
		catch (Exception e)
		{
			M_log.info(this + "transferCopyEntities: End removing Assignmentt data"
					+ e.getMessage());
		}
	...
	/**
	 * This is to mimic the FormattedText.decodeFormattedTextAttribute but use
	 * SAX serialization instead
	 * 
	 * @return
	 */
	protected String formattedTextDecodeFormattedTextAttribute(Attributes attributes,
			String baseAttributeName)
	{
		String ret;

		// first check if an HTML-encoded attribute exists, for example
		// "foo-html", and use it if available
		ret = StringUtils
				.trimToNull(xmlDecodeAttribute(attributes, baseAttributeName + "-html"));
		if (ret != null) return ret;

		// next try the older kind of formatted text like "foo-formatted", and
		// convert it if found
		ret = StringUtils.trimToNull(
				xmlDecodeAttribute(attributes, baseAttributeName + "-formatted"));
		ret = FormattedText.convertOldFormattedText(ret);
		if (ret != null) return ret;

		// next try just a plaintext attribute and convert the plaintext to
		// formatted text if found
		// convert from old plaintext instructions to new formatted text
		// instruction
	...
	protected String xmlDecodeAttribute(Attributes attributes, String tag)
	{
		...
		if (body != null)
		{
			try
			{
				...
				body = org.apache.commons.codec.binary.StringUtils.newString(decoded,
						charset);
			}
	...
	/**
	 * construct the right path for context string, used for permission
	 * checkings
	 * 
	 * @param context
	 * @return
	...
	private LRS_Statement getStatementForAssignmentGraded(LRS_Actor instructor,
			Event event, Assignment a, AssignmentSubmission s, User studentUser)
	{
		LRS_Verb verb = new LRS_Verb(SAKAI_VERB.scored);
		LRS_Object lrsObject = new LRS_Object(
				m_serverConfigurationService.getPortalUrl() + event.getResource(),
				"received-grade-assignment");
		HashMap<String, String> nameMap = new HashMap<String, String>();
		nameMap.put("en-US", "User received a grade");
		lrsObject.setActivityName(nameMap);
		HashMap<String, String> descMap = new HashMap<String, String>();
		descMap.put("en-US", "User received a grade for their assginment: " + a.getTitle()
				+ "; Submission #: " + s.getResubmissionNum());
		...
		LRS_Statement statement = new LRS_Statement(student, verb, lrsObject,
				getLRS_Result(a, s, true), context);
		return statement;
	}

	private LRS_Result getLRS_Result(Assignment a, AssignmentSubmission s,
			boolean completed)
	{
		...
		if (3 == content.getTypeOfGrade() && NumberUtils.isNumber(gradeDisplay))
		{ // Points
			String maxGradePointDisplay = StringUtils
					.replace(content.getMaxGradePointDisplay(), decSeparator, ".");
			result = new LRS_Result(new Float(gradeDisplay), new Float(0.0),
					new Float(maxGradePointDisplay), null);
	...
	private LRS_Statement getStatementForUnsubmittedAssignmentGraded(LRS_Actor instructor,
			Event event, Assignment a, AssignmentSubmission s, User studentUser)
	{
		LRS_Verb verb = new LRS_Verb(SAKAI_VERB.scored);
		LRS_Object lrsObject = new LRS_Object(
				m_serverConfigurationService.getAccessUrl() + event.getResource(),
		...
		HashMap<String, String> descMap = new HashMap<String, String>();
		descMap.put("en-US",
				"User received a grade for an unsubmitted assginment: " + a.getTitle());
		lrsObject.setDescription(descMap);
		LRS_Actor student = new LRS_Actor(studentUser.getEmail());
		student.setName(studentUser.getDisplayName());
		LRS_Context context = new LRS_Context(instructor);
		context.setActivity("other", "assignment");
		LRS_Statement statement = new LRS_Statement(student, verb, lrsObject,
				getLRS_Result(a, s, false), context);
		return statement;
	}
