						...
						Attributes attributes) throws SAXException
				{
					if (doStartElement(uri, localName, qName, attributes))
					{
						if ("content".equals(qName) && entity == null)
						{
							...
							m_checkCompilatio = getBool(attributes.getValue("checkCompilatio"));
                ...
                try {
                    ...
                    if (status != null) {
                        if (status.equals(ContentReviewItem.REPORT_ERROR_NO_RETRY_CODE)) {
							...
							int progression = contentReviewService.getReviewScore(contentId);
							if (progression == 0)
							{	
							...
							{
								errorMessage = rb.getFormattedMessage("content_review.pendingProgression.info",new String[] { String.valueOf(progression) });
							}
                        }
                    }
                    
                    Integer errorCode = contentReviewService.getReviewErrorCode(contentId);
					if (errorCode != null && errorCode > 0)
					{
						if (ContentReviewItem.Error.find(errorCode) != null)
						{
							...
							errorMessage = rb.getString("compilatio." + ContentReviewItem.Error.find(errorCode)) + " " + errorMsg;
						}
					}
			...
			SessionBindingListener
	{
		...
		public String postAttachment(List attachments){
			...
			try {
				...
				if (ass != null && cr != null)
				{
					return contentReviewService.queueContent(null, null, ass.getReference(), cr.getId());
				}
				else
				{
