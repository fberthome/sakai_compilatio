...
public abstract class BaseReviewServiceImpl implements ContentReviewService {
	...
	public String queueContent(String userId, String siteId, String taskId, String contentId)
		...
		ContentReviewItem item = new ContentReviewItem(userId, siteId, taskId, contentId, new Date(),
			ContentReviewItem.NOT_SENDED_CODE);
		item.setNextRetryTime(new Date());
		String errorStr = addDocumentToCompilatio(item);
		int errorCode = -1;
		if (errorStr != null) {
		...
		// FBE : Add Compilatio submission
		log.debug("Add Content To Compilatio: " + contentId);
		return errorStr;
	}

	public String addDocumentToCompilatio(ContentReviewItem contentReviewItem) {
	...
	public Integer getReviewErrorCode(String contentId)
		...
		return ((ContentReviewItem) matchingItems.iterator().next()).getErrorCode();
	}
